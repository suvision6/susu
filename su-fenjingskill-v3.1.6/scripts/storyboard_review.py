#!/usr/bin/env python3
"""Internal source-alignment, format, director-method, and Gate review for su-fenjingskill 3.1.6.

director-workspace/3.1.6 is an internal audit carrier. It never becomes a fifth
...
WORKSPACE_CONTRACT = "director-workspace/3.1.6"
FORMAL_CONTRACT_NAME = "director-shot-data"
FORMAL_CONTRACT_VERSION = "3.1.6"
FORMAL_SOURCE_SKILL = "su-fenjingskill"
FORMAL_SOURCE_SKILL_VERSION = "3.1.6"
...
EVIDENCE_BOILERPLATE_RE = re.compile(
    r"(?:这一变化|该变化|当前变化)?在(?:对白)?D?\d{1,6}(?:发声|说话)?期间继续可见[。；;]*",
    re.IGNORECASE,
)
IDENTIFIER_RE = re.compile(r"(?:D|SH|TU|TB|SC)\d{1,6}", re.IGNORECASE)
ORDINAL_LABEL_RE = re.compile(
    r"第\s*(?:\d+|[零一二三四五六七八九十百]+)\s*(?:镜|镜头|单元|段|步|观看点)"
)
RELATIVE_LABEL_RE = re.compile(r"(?:当前|本)(?:镜头|镜|单元)|(?:上|下|前|后|相邻|下一)单元")
PUNCT_SPACE_RE = re.compile(r"[\s\u3000，。！？；：、,.!?;:‘’“”\"'（）()【】\[\]《》<>—…·｜|/\\_\-]+")


def _strip_dialogue_literals(text: str, dialogue_texts: Iterable[str] = ()) -> str:
    result = text
    for literal in sorted({normalize_text(item) for item in dialogue_texts if normalize_text(item)}, key=len, reverse=True):
        result = result.replace(literal, "")
    return result


def semantic_fingerprint(value: Any, dialogue_texts: Iterable[str] = ()) -> str:
    """Remove identifiers and boilerplate that must not create fake semantic variety."""
    text = normalize_text(value)
    text = _strip_dialogue_literals(text, dialogue_texts)
    text = EVIDENCE_BOILERPLATE_RE.sub("", text)
    text = IDENTIFIER_RE.sub("", text)
    text = ORDINAL_LABEL_RE.sub("", text)
    text = RELATIVE_LABEL_RE.sub("", text)
    return PUNCT_SPACE_RE.sub("", text).casefold()


def evidence_fingerprint(value: Any) -> str:
    """Normalize surface form without deleting unsupported boilerplate claims."""
    text = IDENTIFIER_RE.sub("", normalize_text(value))
    text = ORDINAL_LABEL_RE.sub("", text)
    return PUNCT_SPACE_RE.sub("", text).casefold()


def materially_same(left: str, right: str) -> bool:
    ...
    if len(shorter) >= 12 and shorter in longer:
        return True
    return min(len(left), len(right)) >= 12 and SequenceMatcher(None, left, right).ratio() >= 0.9


def _execution_evidence_texts(shot: Dict[str, Any]) -> list[str]:
    ...
    evidence = [
        staging.get("blocking"), staging.get("performance"), camera.get("focus"),
        movement.get("trigger"), movement.get("path"), movement.get("end_condition"), movement.get("reason"),
    ]
    evidence.extend(sound.get("effects") if isinstance(sound.get("effects"), list) else [])
    for update in continuity.get("state_updates") if isinstance(continuity.get("state_updates"), list) else []:
        if isinstance(update, dict):
            evidence.append("{}{}{}{}".format(update.get("entity", ""), update.get("field", ""), update.get("from", ""), update.get("to", "")))
    return [normalize_text(item) for item in evidence if normalize_text(item)]


def _development_is_realized(development: Any, shot: Dict[str, Any]) -> bool:
    candidate = evidence_fingerprint(development)
    if len(candidate) < 6:
        return False
    for evidence in _execution_evidence_texts(shot):
        actual = evidence_fingerprint(evidence)
        if candidate == actual or (len(candidate) >= 8 and candidate in actual):
            return True
    return False
...
def validate_topology_boundaries(scene_strategies: Any) -> List[Dict[str, str]]:
    ...
    for strategy_index, strategy in enumerate(scene_strategies):
        ...
        seen_boundary_fingerprints: list[tuple[str, str, str]] = []
        for unit_index, unit in enumerate(topology):
            ...
            trigger_fp = semantic_fingerprint(trigger)
            gain_fp = semantic_fingerprint(editorial_gain)
            for previous_trigger, previous_gain, previous_path in seen_boundary_fingerprints:
                if materially_same(trigger_fp, previous_trigger) and materially_same(gain_fp, previous_gain):
                    errors.append(
                        make_issue(
                            "BOUNDARY_TEMPLATE_COLLAPSE",
                            unit_path + ".boundary_to_next",
                            f"切点 trigger 与 editorial_gain 仅靠编号或套话区分，和 {previous_path} 的规范化理由重复。",
                        )
                    )
                    break
            if trigger_fp and gain_fp:
                seen_boundary_fingerprints.append((trigger_fp, gain_fp, unit_path + ".boundary_to_next"))
    return errors
...
def validate_camera_grammar(
    ...
    for strategy_index, strategy in enumerate(scene_strategies):
        ...
        for unit_index, unit in enumerate(topology):
            ...
            viewing_reason = semantic_fingerprint(design.get("reason"))
            if viewing_reason:
            ...
            for shot_id in shot_refs:
                ...
                for category, reason in (
                    ...
                    reason_text = semantic_fingerprint(reason)
                    if reason_text:
        ...
        for shot in scene_shots:
            ...
            camera = shot.get("camera") if isinstance(shot.get("camera"), dict) else {}
            movement = camera.get("movement") if isinstance(camera.get("movement"), dict) else {}
            signatures.append((camera.get("framing_mode"), camera.get("angle"), movement.get("type"), viewpoint.get("owner_type")))
        category_uniform = bool(signatures) and all(len({signature[index] for signature in signatures}) == 1 for index in range(4))
        uniformity_exempt = category_uniform and _camera_reason_specific(grammar.get("uniformity_intent"))

        for category, values in reason_owners.items():
            if uniformity_exempt:
                continue
            for reason, unit_ids in values.items():
                if len(unit_ids) > 1:
                    code = "VIEWING_REASON_TEMPLATE_COLLAPSE" if category == "viewing" else "CAMERA_DECISION_TEMPLATE_COLLAPSE"
                    errors.append(
                        make_issue(
                            code,
                            strategy_path + ".topology",
                            f"不同拓扑单元的 {category} 理由在移除镜号、单元号和套话后重复：{reason}",
                        )
                    )
            reason_items = list(values.items())
            for left_index, (left_reason, left_units) in enumerate(reason_items):
                for right_reason, right_units in reason_items[left_index + 1 :]:
                    if not materially_same(left_reason, right_reason):
                        continue
                    code = "VIEWING_REASON_TEMPLATE_COLLAPSE" if category == "viewing" else "CAMERA_DECISION_TEMPLATE_COLLAPSE"
                    errors.append(
                        make_issue(
                            code,
                            strategy_path + ".topology",
                            "不同拓扑单元的 {} 理由只存在表面改写：{} / {}（单元 {} / {}）。".format(
                                category, left_reason, right_reason, sorted(left_units), sorted(right_units)
                            ),
                        )
                    )
...
def _specific_visible_development(value: Any) -> bool:
    compact = semantic_fingerprint(value)
    generic = {semantic_fingerprint(item) for item in GENERIC_VISIBLE_DEVELOPMENTS}
    return len(compact) >= 6 and compact not in generic
...
def validate_dialogue_edit_plans(
    ...
    for strategy_index, strategy in enumerate(scene_strategies):
        ...
        dialogue_literals = [normalize_text(item.get("text")) for item in dialogue_map.values() if isinstance(item, dict)]
        development_fingerprints: Dict[str, list[str]] = {}
        picture_value_fingerprints: Dict[str, list[str]] = {}
        attention_fingerprints: Dict[str, list[str]] = {}
            ...
            for step_index, step in enumerate(steps):
                ...
                if previous_unit_id is None:
                    ...
                    if not _specific_visible_development(development):
                        ...
                        development_fp = semantic_fingerprint(development, dialogue_literals)
                        previous_developments = development_fingerprints.setdefault(str(unit_id), [])
                        if any(materially_same(development_fp, previous) for previous in previous_developments):
                            errors.append(make_issue("DIALOGUE_HOLD_TEMPLATE_COLLAPSE", step_path + ".visible_development", "不同对白节点不能靠对白 ID、镜号、复述台词或固定尾句制造同一画内发展的伪差异。"))
                        previous_developments.append(development_fp)

                        unit_shots = [formal_shot_map.get(shot_id) for shot_id in unit.get("shot_refs", []) if isinstance(shot_id, str)]
                        if not any(isinstance(shot, dict) and _development_is_realized(development, shot) for shot in unit_shots):
                            errors.append(make_issue("DIALOGUE_VISIBLE_DEVELOPMENT_UNREALIZED", step_path + ".visible_development", "画内发展必须能在本单元正式 blocking、performance、focus、movement、effect 或 state update 中找到可执行证据。"))

                    picture_value_fp = semantic_fingerprint(step.get("picture_value"), dialogue_literals)
                    previous_values = picture_value_fingerprints.setdefault(str(unit_id), [])
                    if picture_value_fp and any(materially_same(picture_value_fp, previous) for previous in previous_values):
                        errors.append(make_issue("DIALOGUE_PICTURE_VALUE_TEMPLATE_COLLAPSE", step_path + ".picture_value", "不同对白节点的画面价值不能只靠对白复述、编号或套话区分。"))
                    if picture_value_fp:
                        previous_values.append(picture_value_fp)

                    attention_fp = semantic_fingerprint(item.get("attention_shift"), dialogue_literals)
                    previous_attention = attention_fingerprints.setdefault(str(unit_id), [])
                    if attention_fp and any(materially_same(attention_fp, previous) for previous in previous_attention):
                        errors.append(make_issue("DIALOGUE_ATTENTION_TEMPLATE_COLLAPSE", item_path + ".attention_shift", "不同对白节点的注意力变化不能只靠复述来源对白或编号制造伪差异。"))
                    if attention_fp:
                        previous_attention.append(attention_fp)
        ...
        if workspace.get("workspace_contract") != WORKSPACE_CONTRACT:
            schema_errors.append(
                make_issue(
                    ...
                    "workspace_contract",
                    "内部工作区必须使用 director-workspace/3.1.6。",
                )
            )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="验证并操作 su-fenjingskill 3.1.6 的格式、来源、导演方法、Gate 与 alignment 审批链。"
    )
    ...
    validate_parser = subparsers.add_parser("validate", help="验证内部工作区与正式 3.1.6 shot data。")
