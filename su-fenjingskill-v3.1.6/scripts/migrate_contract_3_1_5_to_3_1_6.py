#!/usr/bin/env python3
"""Create a non-overwriting 3.1.6 retiming draft from 3.1.5 inputs."""
...
SCRIPT_INTERFACE_REASON = "Explicit non-overwriting 3.1.5 to 3.1.6 retiming draft migration."
...
def _validate_identity(shot_data: Dict[str, Any], workspace: Dict[str, Any]) -> None:
    expected = {
        ...
        "contract_version": "3.1.5",
        "source_skill": "su-fenjingskill",
        "source_skill_version": "3.1.5",
    }
    for key, value in expected.items():
        if shot_data.get(key) != value:
            raise ValueError(f"迁移输入 {key} 必须是 {value!r}")
    if workspace.get("workspace_contract") != "director-workspace/3.1.5":
        raise ValueError("--workspace 必须是 director-workspace/3.1.5")
    ...
    if not formal_text or formal_text != workspace_text:
        raise ValueError("3.1.5 workspace 与 shot data 的 locked_text 缺失或不一致")
...
def build_drafts(
    ...
    shot_draft["validation"] = {
        "status": "draft",
        "warnings": [
            "MIGRATION_RETIMING_REQUIRED: formal shot durations must be measured to whole seconds with a one-second minimum.",
            "MIGRATION_REVIEW_REQUIRED: one- and two-second shots need observable bounds and a concrete compression function.",
            "MIGRATION_REVIEW_REQUIRED: Alignment is invalidated; Gate 0, Gate 1 and Gate 2 are preserved.",
        ],
    }
    ...
    workspace_draft["approval_events"] = [
        copy.deepcopy(event)
        for event in events
        if isinstance(event, dict) and event.get("event_type") != "alignment_approved"
    ]
    ...
    review_lock.update({
        ...
        "alignment_note": "3.1.5 shot timing requires whole-second retiming and ultra-short review under 3.1.6",
    })
    workspace_draft["review_lock"] = review_lock

    fractional_ids: list[str] = []
    below_minimum_ids: list[str] = []
    ...
    for shot in shot_draft.get("shots", []):
        ...
        if isinstance(duration, (int, float)) and not isinstance(duration, bool):
            if float(duration) < 1:
                below_minimum_ids.append(shot_id)
            elif not _is_whole_second(duration):
                fractional_ids.append(shot_id)
            if 1 <= float(duration) <= 2:
    ...
    report = {
        "migration": "director-shot-data/3.1.5 -> director-shot-data/3.1.6",
        "workspace_migration": "director-workspace/3.1.5 -> director-workspace/3.1.6",
        "status": "DRAFT_RETIMING_REQUIRED",
        "formal_ready": False,
        ...
        "preserved": ["locked source", "format brief", "director method", "scene strategy", "Gate 0", "Gate 1", "Gate 2", "formal shot content and timing measurements"],
        "review_stages_invalidated": ["alignment"],
        "fractional_duration_shot_ids": fractional_ids,
        "below_minimum_shot_ids": below_minimum_ids,
        "ultra_short_shot_ids": ultra_short_ids,
        "next_steps": [
            "remeasure fractional or sub-second shots against their actual executable events; do not round them mechanically",
            "give every one- or two-second shot observable start, end and compression-function evidence",
            "rerun 3.1.6 validation and approve Alignment only after retiming passes",
        ],
    }
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="非覆盖迁移 3.1.5 到 3.1.6 整秒重测草稿。")
...
def main(argv: Optional[Iterable[str]] = None) -> int:
    ...
    try:
        ...
        values = {"shot_data_draft": shot_draft, "workspace_draft": workspace_draft, "migration_report": report}
        for key, path in paths.items():
            write_json_new(path, values[key])
            sys.stdout.write(f"{key}: {path}\n")
        sys.stdout.write("status: DRAFT_RETIMING_REQUIRED\n")
        return 0
