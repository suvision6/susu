#!/usr/bin/env python3
"""Deterministic human-facing XLSX renderer for director-shot-data/3.1.6.
...
def build_openpyxl_workbook(data: dict[str, Any]):
    ...
    for row_number in range(5, storyboard.max_row + 1):
        ...
        storyboard.cell(row=row_number, column=4).number_format = '0"秒"'
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="将通过 combined gate 的 director-shot-data/3.1.6 导出为 XLSX。")
    parser.add_argument("--input", type=Path, required=True, help="输入 JSON 文件。")
    parser.add_argument("--workspace", type=Path, required=True, help="director-workspace/3.1.6。")
