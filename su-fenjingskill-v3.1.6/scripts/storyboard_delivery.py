#!/usr/bin/env python3
"""Formal director-shot-data/3.1.6 backend shipped by su-fenjingskill 3.1.6.
...
try:  # supports both script-path and ``python -m scripts...`` execution
    ...
    from ._xlsx_projection import (
        XlsxProjectionError,
        is_canonical_shot_size,
        projection_optional_items,
...
CONTRACT_VERSION = "3.1.6"
SOURCE_SKILL = "su-fenjingskill"
SOURCE_SKILL_VERSION = "3.1.6"
...
RHYTHM_PROFILES = {
    "short_drama_under_10m",
    "platform_series_episode",
    "short_film_3_20m",
    "feature_film",
    "custom",
}
...
WHOLE_SECOND_EPSILON = 1e-9
ULTRA_SHORT_MAX_SECONDS = 2
ULTRA_SHORT_START_CUES = (
    "从", "当", "随", "起点", "开始", "切入", "声起", "动作起", "from", "when", "as ",
)
ULTRA_SHORT_END_CUES = (
    "直到", "至", "结束", "停止", "停住", "落下", "完成", "看清", "读清", "出现", "离开", "切出", "切走", "until", "ends", "stops", "lands", "completes",
)
ULTRA_SHORT_FUNCTION_CUES = (
    "瞬时", "短促", "只读取", "单一", "冲击", "接点", "闪回", "碎片", "断裂", "蒙太奇", "危险", "强调", "突然", "立即", "反转", "包袱", "节奏", "过渡", "方向", "动作结果", "反应", "信息落点", "瞬间", "single", "impact", "fragment", "transition",
)
...
def _close_seconds(left: Any, right: Any, tolerance: float = 0.02) -> bool:
    try:
...
def _is_whole_second(value: Any) -> bool:
    try:
        number = float(value)
    except (TypeError, ValueError):
        return False
    return abs(number - round(number)) <= WHOLE_SECOND_EPSILON


def _ultra_short_basis_is_complete(value: Any) -> bool:
    """Require observable bounds and a compression function for 1–2 second shots."""
    text = normalize_text(value).casefold()
    return (
        any(cue in text for cue in ULTRA_SHORT_START_CUES)
        and any(cue in text for cue in ULTRA_SHORT_END_CUES)
        and any(cue in text for cue in ULTRA_SHORT_FUNCTION_CUES)
    )


def _validate_format_brief(data: dict[str, Any], errors: list[dict[str, str]]) -> dict[str, Any]:
    brief = data.get("format_brief")
    if not isinstance(brief, dict):
        errors.append(issue("FORMAT_BRIEF_REQUIRED", "format_brief", "拆镜前必须确认节奏、目标时长、画幅、方向和对白速度。"))
        return {}
    ...
    if profile not in RHYTHM_PROFILES:
        errors.append(issue("RHYTHM_PROFILE_INVALID", "format_brief.rhythm_profile", "剪辑节奏预设无效。"))
    if not isinstance(runtime, (int, float)) or isinstance(runtime, bool) or runtime <= 0:
        errors.append(issue("TARGET_RUNTIME_INVALID", "format_brief.target_runtime_seconds", "目标成片时长必须大于 0。"))
    elif profile == "short_drama_under_10m" and runtime > 600:
        errors.append(issue("RHYTHM_PROFILE_RUNTIME_MISMATCH", "format_brief.target_runtime_seconds", "十分钟以内短剧预设的目标时长不能超过 600 秒。"))
    elif profile == "short_film_3_20m" and not 180 <= runtime <= 1200:
        errors.append(issue("RHYTHM_PROFILE_RUNTIME_MISMATCH", "format_brief.target_runtime_seconds", "3–20 分钟短片预设的目标时长必须位于 180–1200 秒。"))
    if ratio not in ASPECT_RATIOS:
        errors.append(issue("ASPECT_RATIO_INVALID", "format_brief.aspect_ratio", "画幅比例无效。"))
    if orientation not in ORIENTATIONS:
        errors.append(issue("ORIENTATION_INVALID", "format_brief.orientation", "画面方向无效。"))
    if ratio == "9:16" and orientation != "vertical":
        errors.append(issue("ASPECT_ORIENTATION_MISMATCH", "format_brief.orientation", "9:16 必须确认为竖屏方向。"))
    if ratio in {"16:9", "1.85:1", "2.35:1", "2.39:1", "4:3"} and orientation != "horizontal":
        errors.append(issue("ASPECT_ORIENTATION_MISMATCH", "format_brief.orientation", f"{ratio} 必须确认为横屏方向。"))
    pace = brief.get("dialogue_pace")
    if not isinstance(pace, dict):
        errors.append(issue("DIALOGUE_PACE_REQUIRED", "format_brief.dialogue_pace", "Gate 0 必须确认可计算的对白速度参数。"))
        return {}
    for code, relative_path, message in dialogue_pace_issues(profile, pace):
        suffix = f".{relative_path}" if relative_path else ""
        errors.append(issue(code, "format_brief.dialogue_pace" + suffix, message))
    return pace
...
def _validate_timing_plan(
    ...
    plan = shot.get("timing_plan")
    flow = shot.get("shot_flow") if isinstance(shot.get("shot_flow"), list) else []
    ...
    if not isinstance(plan, dict):
        errors.append(issue("TIMING_PLAN_REQUIRED", f"{path}.timing_plan", "镜头必须用结构化时间块说明时长。"))
        return
    if plan.get("status") != "ready":
        errors.append(issue("TIMING_PLAN_UNRESOLVED", f"{path}.timing_plan.status", "正式镜头 timing_plan 必须完成重建并标记为 ready。"))
    blocks = plan.get("blocks")
    if not isinstance(blocks, list) or not blocks:
        errors.append(issue("TIMING_BLOCKS_REQUIRED", f"{path}.timing_plan.blocks", "timing_plan 必须包含时间块。"))
        return
    expected_start = 0
    block_ids: set[str] = set()
    computed_total = 0.0
    components = ("dialogue_seconds", "action_seconds", "camera_seconds", "sound_seconds", "reaction_seconds", "hold_seconds")
    for index, block in enumerate(blocks):
        block_path = f"{path}.timing_plan.blocks[{index}]"
        if not isinstance(block, dict):
            errors.append(issue("TIMING_BLOCK_INVALID", block_path, "时间块必须是对象。"))
            continue
        block_id = block.get("block_id")
        if not isinstance(block_id, str) or block_id in block_ids:
            errors.append(issue("TIMING_BLOCK_ID_INVALID", block_path + ".block_id", "时间块 ID 为空或重复。"))
        else:
            block_ids.add(block_id)
        start = block.get("flow_start_index")
        end = block.get("flow_end_index")
        if type(start) is not int or type(end) is not int or start != expected_start or end < start or end >= len(flow):
            errors.append(issue("TIMING_FLOW_COVERAGE_INVALID", block_path, "时间块必须无缝、无重叠地覆盖全部 shot_flow。"))
            continue
        values: dict[str, float] = {}
        for field in components:
            value = block.get(field)
            if not isinstance(value, (int, float)) or isinstance(value, bool) or value < 0:
                errors.append(issue("TIMING_COMPONENT_INVALID", block_path + "." + field, "时间分量必须是非负数。"))
                value = 0
            values[field] = float(value)
        owners = [item.get("owner") for item in flow[start : end + 1] if isinstance(item, dict)]
        texts: list[str] = []
        dialogue_ids: list[str] = []
        for item in flow[start : end + 1]:
            if not isinstance(item, dict) or item.get("owner") != "dialogue_segment":
                continue
            segment_index = item.get("index")
            if type(segment_index) is int and 0 <= segment_index < len(dialogue_segments):
                segment = dialogue_segments[segment_index] if isinstance(dialogue_segments[segment_index], dict) else {}
        ...
        effective_pace = resolve_dialogue_pace(pace, block.get("pace_ref"))
        if effective_pace is None:
            errors.append(issue("TIMING_PACE_REF_INVALID", block_path + ".pace_ref", "pace_ref 必须引用 BASE 或一个已登记的 pace override。"))
            effective_pace = {}
        elif effective_pace.get("scope") == "scene" and effective_pace.get("scope_ref") != shot.get("scene_id"):
            errors.append(issue("TIMING_PACE_SCOPE_MISMATCH", block_path + ".pace_ref", "scene pace override 与当前镜头 scene_id 不一致。"))
        elif effective_pace.get("scope") == "dialogue" and set(dialogue_ids) != {effective_pace.get("scope_ref")}:
            errors.append(issue("TIMING_PACE_SCOPE_MISMATCH", block_path + ".pace_ref", "dialogue pace override 只能用于仅包含该 dialogue_id 的时间块。"))
        elif effective_pace.get("scope") == "character":
            speakers = {
                dialogue_map.get(dialogue_id, {}).get("speaker")
                for dialogue_id in dialogue_ids
                if dialogue_id in dialogue_map
            }
            if speakers != {effective_pace.get("scope_ref")}:
                errors.append(issue("TIMING_PACE_SCOPE_MISMATCH", block_path + ".pace_ref", "character pace override 只能用于该角色的对白时间块。"))
        chinese_text = "".join(text for text in texts if contains_cjk(text))
        expected_dialogue = chinese_dialogue_estimated_seconds(chinese_text, effective_pace) if chinese_text else 0.0
        if chinese_text and not _close_seconds(values["dialogue_seconds"], expected_dialogue):
            errors.append(issue("DIALOGUE_TIMING_MISMATCH", block_path + ".dialogue_seconds", f"对白时间必须按 Gate 0 速度参数重算，当前应约为 {expected_dialogue:.3f} 秒。"))
        owner_set = set(owners)
        if "blocking" in owners and values["action_seconds"] <= 0:
            errors.append(issue("ACTION_TIMING_MISSING", block_path + ".action_seconds", "时间块包含调度动作，必须估计 action_seconds。"))
        if "movement" in owners and values["camera_seconds"] <= 0:
            errors.append(issue("CAMERA_TIMING_MISSING", block_path + ".camera_seconds", "时间块包含摄影机运动，必须估计 camera_seconds。"))
        if "performance" in owners and values["reaction_seconds"] <= 0 and values["hold_seconds"] <= 0:
            errors.append(issue("REACTION_TIMING_MISSING", block_path + ".reaction_seconds", "时间块包含表演／反应，必须估计 reaction_seconds。"))
        if any(owner in {"effect", "ambience"} for owner in owners) and values["sound_seconds"] <= 0:
            errors.append(issue("SOUND_TIMING_MISSING", block_path + ".sound_seconds", "时间块包含声音事件，必须估计 sound_seconds。"))

        component_owner_rules = {
            "dialogue_seconds": {"dialogue_segment"},
            "action_seconds": {"blocking", "state_update"},
            "camera_seconds": {"movement"},
            "reaction_seconds": {"performance"},
            "sound_seconds": {"effect", "ambience"},
        }
        for component, allowed_owners in component_owner_rules.items():
            if values[component] > 0 and not owner_set.intersection(allowed_owners):
                errors.append(
                    issue(
                        "TIMING_COMPONENT_FLOW_MISMATCH",
                        block_path + "." + component,
                        f"{component} 为正，但当前 flow 范围没有对应事件 owner：{sorted(allowed_owners)}。",
                    )
                )

        if values["hold_seconds"] > 0:
            active_components = [
                field for field in ("dialogue_seconds", "action_seconds", "camera_seconds", "reaction_seconds")
                if values[field] > 0
            ]
            if active_components:
                errors.append(
                    issue(
                        "TIMING_HOLD_ACTIVE_CONFLICT",
                        block_path + ".hold_seconds",
                        "hold_seconds 只表示独立静默、等待、凝视、停滞或余波；不能与对白、动作、运镜或反应共享同一时间块。",
                    )
                )
            forbidden_hold_owners = owner_set.intersection({"dialogue_segment", "blocking", "movement", "state_update"})
            supporting_hold_owners = owner_set.intersection({"performance", "focus", "ambience", "edit_exit"})
            basis_text = normalize_text(shot.get("duration_basis")) + " " + normalize_text(plan.get("basis"))
            has_start = any(term in basis_text for term in ("从", "当", "开始", "起点", "进入"))
            has_end = any(term in basis_text for term in ("直到", "至", "结束", "停止", "落下", "开口", "转身", "离开", "终点"))
            has_function = any(term in basis_text for term in ("余波", "等待", "凝视", "静默", "压力", "尴尬", "关系", "信息", "反应", "悬置", "威胁", "笑点", "观众"))
            if forbidden_hold_owners or not supporting_hold_owners or not (has_start and has_end and has_function):
                errors.append(
                    issue(
                        "TIMING_HOLD_EVIDENCE_MISSING",
                        block_path + ".hold_seconds",
                        "独立停留块必须只覆盖可支持停留的 flow，并在时长依据中说明可观察起点、结束条件与戏剧功能。",
                    )
                )
        mode = block.get("mode")
        expected_block = sum(values.values()) if mode == "sequential" else max(values.values()) if mode == "parallel" else 0.0
        if mode not in {"sequential", "parallel"}:
            errors.append(issue("TIMING_MODE_INVALID", block_path + ".mode", "时间块必须说明 sequential 或 parallel。"))
        if not _close_seconds(block.get("computed_seconds"), expected_block):
            errors.append(issue("TIMING_BLOCK_SUM_MISMATCH", block_path + ".computed_seconds", f"时间块计算结果应为 {expected_block:.3f} 秒。"))
        computed_total += expected_block
        expected_start = end + 1
    if expected_start != len(flow):
        errors.append(issue("TIMING_FLOW_COVERAGE_INVALID", f"{path}.timing_plan.blocks", "timing blocks 未覆盖全部 shot_flow。"))
    if not _close_seconds(plan.get("computed_duration_seconds"), computed_total):
        errors.append(issue("TIMING_PLAN_SUM_MISMATCH", f"{path}.timing_plan.computed_duration_seconds", f"timing_plan 总时长应为 {computed_total:.3f} 秒。"))
    if plan.get("status") == "ready" and not _is_whole_second(computed_total):
        errors.append(
            issue(
                "TIMING_PLAN_NOT_WHOLE_SECOND",
                f"{path}.timing_plan.computed_duration_seconds",
                f"正式 timing plan 合计为 {computed_total:.3f} 秒，必须通过真实事件测量调整到整秒；不得直接四舍五入镜长。",
            )
        )
    if numeric_duration and not _close_seconds(numeric_duration, computed_total):
        errors.append(issue("SHOT_DURATION_MISMATCH", f"{path}.duration_seconds", f"镜头时长必须等于时间块总和 {computed_total:.3f} 秒。"))
...
def _camera_design_issues(
    ...
    enum_fields = (
        ...
        (camera, "frame_axis", FRAME_AXES, f"{path}.camera.frame_axis"),
    )
    ...
    for value, field_path, label in (
        ...
        (camera.get("aspect_ratio_reason"), f"{path}.camera.aspect_ratio_reason", "画幅适配理由"),
    ...
    aspect_reason = normalize_text(camera.get("aspect_ratio_reason"))
    if aspect_reason and (len("".join(aspect_reason.split())) < 8 or aspect_reason in {"适配画幅", "符合画幅", "画幅需要"}):
        errors.append(issue("ASPECT_RATIO_APPLICATION_VAGUE", f"{path}.camera.aspect_ratio_reason", "必须说明当前构图或运动如何使用已确认画幅，不能只写适配画幅。"))
...
def validate_structure(data: Any, *, allow_legacy: bool = False) -> dict[str, Any]:
    ...
    dialogue_pace = _validate_format_brief(data, errors)
    ...
    movements: list[str] = []
    total_duration = 0.0
    ultra_short_shot_ids: list[str] = []
    projected_optional_by_scene: dict[str, dict[str, dict[str, list[str]]]] = defaultdict(
    ...
    for index, shot in enumerate(shots, start=1):
        ...
        if not isinstance(duration, (int, float)) or isinstance(duration, bool):
            errors.append(issue("DURATION_INVALID", f"{path}.duration_seconds", "镜头时长必须是数值。"))
        else:
            numeric_duration = float(duration)
            if numeric_duration > 0:
                total_duration += numeric_duration
            if numeric_duration < 1:
                errors.append(issue("SHOT_DURATION_BELOW_MINIMUM", f"{path}.duration_seconds", "正式镜头最短为 1 秒。"))
            elif not _is_whole_second(numeric_duration):
                errors.append(issue("SHOT_DURATION_NOT_WHOLE_SECOND", f"{path}.duration_seconds", "正式镜头时长必须是整秒数；时间块内部可保留小数测量，但合计必须落到整数秒。"))
            elif numeric_duration <= ULTRA_SHORT_MAX_SECONDS:
                ultra_short_shot_ids.append(str(shot_id))
                if not _ultra_short_basis_is_complete(shot.get("duration_basis")):
                    errors.append(
                        issue(
                            "ULTRA_SHORT_SHOT_EVIDENCE_MISSING",
                            f"{path}.duration_basis",
                            "1–2 秒超短镜必须说明可观察起点、结束条件，以及瞬时信息、动作接点、冲击、断裂或其他短促功能。",
                        )
                    )
                warnings.append(
                    issue(
                        "ULTRA_SHORT_SHOT_REVIEW_REQUIRED",
                        f"{path}.duration_seconds",
                        f"{shot_id} 为 {int(numeric_duration)} 秒超短镜；数字本身不判错，但须在人工审阅中确认读取时间与整场节奏。",
                    )
                )
        ...
        if get_nonempty_string(camera, "shot_size") and not is_canonical_shot_size(camera.get("shot_size")):
            errors.append(
                issue(
                    "SHOT_SIZE_TERM_NONSTANDARD",
                    f"{path}.camera.shot_size",
                    "景别必须使用大远景、远景、中远景、全景、中全景、中景、中近景、近景、特写、大特写或极特写；镜内变化用→连接。",
                )
            )
        ...
        if not isinstance(execution_text, str) or not execution_text.strip():
            ...
            required_sections = ("【观看】", "【摄影】", "【调度与表演】", "【声音】", "【剪辑】", "【连续性】", "【时长】", "【镜头动机】")
        ...
        _validate_timing_plan(shot, dialogue_pace, dialogue_map, path, numeric_duration, errors)
        ...
        try:
            ...
            for owner, optional_text in projection_optional_items(shot):
                normalized_optional = "".join(optional_text.split())
                projected_optional_by_scene[str(scene_id)][owner][normalized_optional].append(str(shot_id))
        except XlsxProjectionError as exc:
            errors.append(
                issue(
                    getattr(exc, "code", "XLSX_PROJECTION_INVALID"),
                    f"{path}.shot_flow",
                    str(exc),
                )
            )

    for scene_id, owners in projected_optional_by_scene.items():
        for owner, values in owners.items():
            for repeated_text, shot_ids in values.items():
                if len(shot_ids) <= 1:
                    continue
                errors.append(
                    issue(
                        "XLSX_PROJECTION_REPEATED_OPTIONAL_FLOW",
                        f"scenes[{scene_id}].shot_flow",
                        f"{owner} 在多个镜头重复进入 XLSX（{', '.join(shot_ids)}）：{repeated_text}。重复的普通环境、焦点或结束说明应只留在后台。",
                    )
                )

    current_run: list[str] = []
    ultra_short_runs: list[list[str]] = []
    ultra_short_set = set(ultra_short_shot_ids)
    previous_scene_id = None
    for shot in shots:
        shot_id = str(shot.get("shot_id", "")) if isinstance(shot, dict) else ""
        ...
        if previous_scene_id is not None and scene_id != previous_scene_id:
            if len(current_run) >= 2:
                ultra_short_runs.append(current_run)
            current_run = []
        if shot_id in ultra_short_set:
            current_run.append(shot_id)
        else:
            if len(current_run) >= 2:
                ultra_short_runs.append(current_run)
            current_run = []
    ...
    if len(current_run) >= 2:
        ultra_short_runs.append(current_run)
    for run in ultra_short_runs:
        warnings.append(
            issue(
                "ULTRA_SHORT_SHOT_CLUSTER_REVIEW_REQUIRED",
                "shots",
                f"连续超短镜需人工确认是否确有密集切换功能：{', '.join(run)}。该提示不按数量或比例自动判失败。",
            )
        )
    ...
    brief = data.get("format_brief") if isinstance(data.get("format_brief"), dict) else {}
    pace = brief.get("dialogue_pace") if isinstance(brief.get("dialogue_pace"), dict) else {}
    result["summary"].update(
        {
            "rhythm_profile": brief.get("rhythm_profile"),
            "aspect_ratio": brief.get("aspect_ratio"),
            "orientation": brief.get("orientation"),
            "dialogue_pace_profile": pace.get("pace_profile"),
            "dialogue_pace_range_cps": [pace.get("range_min_cps"), pace.get("range_max_cps")],
            "dialogue_selected_cps": pace.get("selected_cps"),
            "timing_block_count": sum(
                len(shot.get("timing_plan", {}).get("blocks", []))
                for shot in shots
                if isinstance(shot, dict) and isinstance(shot.get("timing_plan"), dict)
            ),
            **_timing_and_density_diagnostics(data),
        }
    )
    return result


def _timing_and_density_diagnostics(data: dict[str, Any]) -> dict[str, Any]:
    """Expose review signals without turning density or duration into artistic quotas."""
    shots = data.get("shots") if isinstance(data.get("shots"), list) else []
    dialogue_map = {
        item.get("dialogue_id"): item
        for item in (data.get("source", {}).get("dialogue_lines", []) if isinstance(data.get("source"), dict) else [])
        if isinstance(item, dict)
    }
    actual = round(sum(float(shot.get("duration_seconds", 0)) for shot in shots if isinstance(shot, dict)), 3)
    brief = data.get("format_brief") if isinstance(data.get("format_brief"), dict) else {}
    target = brief.get("target_runtime_seconds")
    delta = round(actual - float(target), 3) if isinstance(target, (int, float)) and not isinstance(target, bool) else None
    delta_percent = round((delta / float(target)) * 100, 3) if delta is not None and float(target) != 0 else None
    single_parallel = 0
    hold_dominant = 0
    complex_single_take = 0
    ultra_short_ids: list[str] = []
    ultra_short_runs: list[list[str]] = []
    current_ultra_short_run: list[str] = []
    previous_scene_id = None
    scene_metrics: dict[str, dict[str, int]] = {}
    for shot in shots:
        if not isinstance(shot, dict):
            continue
        scene_id = str(shot.get("scene_id", ""))
        if previous_scene_id is not None and scene_id != previous_scene_id:
            if len(current_ultra_short_run) >= 2:
                ultra_short_runs.append(current_ultra_short_run)
            current_ultra_short_run = []
        metrics = scene_metrics.setdefault(scene_id, {
            "shot_count": 0, "dialogue_count": 0, "speaker_handoff_count": 0, "intra_shot_reframe_count": 0,
            "ultra_short_shot_count": 0,
        })
        metrics["shot_count"] += 1
        duration = shot.get("duration_seconds")
        if isinstance(duration, (int, float)) and not isinstance(duration, bool) and 1 <= float(duration) <= ULTRA_SHORT_MAX_SECONDS:
            shot_id = str(shot.get("shot_id", ""))
            ultra_short_ids.append(shot_id)
            current_ultra_short_run.append(shot_id)
            metrics["ultra_short_shot_count"] += 1
        else:
            if len(current_ultra_short_run) >= 2:
                ultra_short_runs.append(current_ultra_short_run)
            current_ultra_short_run = []
        previous_scene_id = scene_id
        sound = shot.get("sound") if isinstance(shot.get("sound"), dict) else {}
        segments = [item for item in sound.get("dialogue_segments", []) if isinstance(item, dict)] if isinstance(sound.get("dialogue_segments"), list) else []
        speakers = [dialogue_map.get(item.get("dialogue_id"), {}).get("speaker") for item in segments]
        handoffs = sum(1 for left, right in zip(speakers, speakers[1:]) if left and right and left != right)
        metrics["dialogue_count"] += len(segments)
        metrics["speaker_handoff_count"] += handoffs
        if len(segments) >= 3 or handoffs >= 2:
            complex_single_take += 1
        viewpoint = shot.get("viewpoint") if isinstance(shot.get("viewpoint"), dict) else {}
        camera = shot.get("camera") if isinstance(shot.get("camera"), dict) else {}
        movement = camera.get("movement") if isinstance(camera.get("movement"), dict) else {}
        if viewpoint.get("camera_response") in {"reframe", "follow", "reveal"} or movement.get("type") not in {None, "fixed"}:
            metrics["intra_shot_reframe_count"] += 1
        plan = shot.get("timing_plan") if isinstance(shot.get("timing_plan"), dict) else {}
        blocks = [block for block in plan.get("blocks", []) if isinstance(block, dict)] if isinstance(plan.get("blocks"), list) else []
        if len(blocks) == 1 and blocks[0].get("mode") == "parallel":
            single_parallel += 1
        for block in blocks:
            values = [float(block.get(field, 0) or 0) for field in (
                "dialogue_seconds", "action_seconds", "camera_seconds", "sound_seconds", "reaction_seconds", "hold_seconds"
            )]
            hold = values[-1]
            if hold > 0 and hold == max(values):
                hold_dominant += 1
    if len(current_ultra_short_run) >= 2:
        ultra_short_runs.append(current_ultra_short_run)
    return {
        ...
        "runtime_delta_percent": delta_percent,
        "single_parallel_block_shot_count": single_parallel,
        "hold_dominant_block_count": hold_dominant,
        "complex_single_take_count": complex_single_take,
        "ultra_short_shot_count": len(ultra_short_ids),
        "ultra_short_shot_ids": ultra_short_ids,
        "ultra_short_run_count": len(ultra_short_runs),
        "max_ultra_short_run_length": max((len(run) for run in ultra_short_runs), default=0),
        "ultra_short_runs": ultra_short_runs,
        "scene_review_metrics": scene_metrics,
    }
...
def _combined_report(
    ...
    *,
    ...
    if workspace_missing:
        errors.append(issue("WORKSPACE_REQUIRED", "workspace", "3.1.6 正式 validate/build/export 必须提供 director-workspace/3.1.6。"))
...
def render_markdown(data: dict[str, Any], report: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"- 画幅：`{data.get('format_brief', {}).get('aspect_ratio', '')}`",
        f"- 节奏预设：`{data.get('format_brief', {}).get('rhythm_profile', '')}`",
        f"- 对白速度：`{data.get('format_brief', {}).get('dialogue_pace', {}).get('pace_profile', '')} / {data.get('format_brief', {}).get('dialogue_pace', {}).get('selected_cps', '')} 字/秒（区间 {data.get('format_brief', {}).get('dialogue_pace', {}).get('range_min_cps', '')}–{data.get('format_brief', {}).get('dialogue_pace', {}).get('range_max_cps', '')}）`",
        f"- 语速标准：`{data.get('format_brief', {}).get('dialogue_pace', {}).get('standard_id', '')}`",
        f"- 语速口径：`{data.get('format_brief', {}).get('dialogue_pace', {}).get('rate_basis', '')}`",
        "",
        "## 导演设计摘要",
        "",
        "| 维度 | 设计 |",
...
def verify_markdown(data: dict[str, Any], path: Path) -> dict[str, Any]:
    ...
    format_brief = data.get("format_brief") if isinstance(data.get("format_brief"), dict) else {}
    pace = format_brief.get("dialogue_pace") if isinstance(format_brief.get("dialogue_pace"), dict) else {}
    for value in (
        format_brief.get("aspect_ratio"),
        format_brief.get("rhythm_profile"),
        pace.get("pace_profile"),
        pace.get("standard_id"),
        pace.get("rate_basis"),
        pace.get("range_min_cps"),
        pace.get("range_max_cps"),
        pace.get("selected_cps"),
    ):
        normalized = normalize_text(value)
        if normalized and normalized not in text:
            raise ValueError("Markdown 未完整承载 Gate 0 格式与节奏事实。")
    assumptions = data.get("assumptions") if isinstance(data.get("assumptions"), list) else []
    for item in assumptions:
        if not isinstance(item, dict):
            continue
...
def build_outputs(
    ...
    *,
    ...
    ``strict`` is retained for call-site compatibility; 3.1.6 is always fail-closed.
    """
    del strict
    ...
    if output_dir.exists() and (not output_dir.is_dir() or any(output_dir.iterdir())):
        blocking = issue(
            ...
            "output_dir",
            "3.1.6 正式 build-all 只允许不存在或为空的输出目录；拒绝覆盖任何已有文件。",
        )
        failed = copy.deepcopy(report)
...
def cmd_build(args: argparse.Namespace) -> int:
    if args.output_dir.exists() and (not args.output_dir.is_dir() or any(args.output_dir.iterdir())):
        sys.stderr.write("FAIL: 3.1.6 正式 build-all 输出目录必须不存在或为空；未修改任何已有文件。\n")
        return 1
    try:
        data = load_json(args.input)
        workspace = load_json(args.workspace) if args.workspace else None
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="su-fenjingskill 3.1.6 后端：Gate 0、对白观看计划、结构化时长、来源保护与原子四文件交付。"
    )
    ...
    validate_parser = subparsers.add_parser("validate", help="组合校验 3.1.6 workspace 与正式 shot data。")
    validate_parser.add_argument("--input", type=Path, required=True, help="输入 JSON 文件。")
    validate_parser.add_argument("--workspace", type=Path, help="director-workspace/3.1.6；正式 readiness 必需。")
    ...
    for command in ("build-all", "build"):
        ...
        build_parser_.add_argument("--workspace", type=Path, required=True, help="director-workspace/3.1.6。")
