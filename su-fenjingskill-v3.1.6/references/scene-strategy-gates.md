...
- `target_runtime_seconds`：项目目标总时长；十分钟短剧不得超过 600 秒，3–20 分钟短片必须位于 180–1200 秒。它只在逐镜测量后用于核对，不得反向分配镜头、停留或反应秒数；
...
同一 topology unit 内只能 `hold` 或 `reframe`。继续不切时，每一轮换人对白都必须出现新的可见动作、反应、距离、焦点或关系变化；该发展必须能在正式 blocking、performance、focus、movement、effect 或 state update 中找到执行证据。“继续反应”“保持画面”等套话触发 `MISSED_DIALOGUE_VIEW_CHANGE`；追加对白 ID、镜号、复述台词或固定尾句后仍相同的内容触发模板坍缩。进入新的 topology unit 必须标记 `cut`；首个对白画面步骤使用 `establish`。
...
不设置单人、景别、角度、运动或 framing 数量配额。若整场 framing、角度、运动和画面所有者全部相同，`uniformity_intent` 必须针对当前场景事件和方法说明为什么统一优于变化，并由用户重新确认 Gate 2；缺失或空泛时不能正式构建。观看、摄影与切点理由比较前移除对白 ID、镜号、单元号和相对编号；规范化后相同或高度近似仍触发模板坍缩。周期轮换类别不能替代导演理由。真实仪式重复或同质摄影可由具体 `uniformity_intent` 保护，不能只靠改编号通过。
...
机器检查 Schema、哈希、引用、审批状态、viewing design／正式执行一致性、编号与套话逃逸、无执行证据的对白留镜、无事件停留和确定性矛盾。题材机制、具体摄影审美和长镜／跳切／蒙太奇是否有效，仍由导演或用户复核；镜头数、镜长和统计比例不得替代判断。
