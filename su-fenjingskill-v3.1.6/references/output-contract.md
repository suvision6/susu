# Output Contract｜director-shot-data/3.1.6
...
内部来源台账、事实、镜头绑定、Gate 和确认事件属于 `director-workspace/3.1.6`，不进入正式数据文件。3.1.5 只能通过显式迁移生成新目录中的 3.1.6 draft，不能只改版本字符串。
...
目标总时长只用于完成逐镜测量后的差值报告，不得反向分配到 shot 或 hold。正式 `duration_seconds` 必须是大于等于 1 的整数秒；各时间分量可以保留小数，但全部 blocks 的真实合计也必须落到该整数。不得把小数测量结果直接四舍五入，而要回到可执行事件，明确使镜头边界成为该整数秒的动作、反应、声音、运镜或独立停留。

`hold_seconds` 只属于独立静默、等待、凝视、停滞或余波块；同块含对白、动作、运镜或反应时必须为 0。每个正时间分量必须由当前 block 覆盖的 shot_flow owner 支持，后续停顿另建 block。
...
1–2 秒镜头必须在 `duration_basis` 中写明可观察起点、结束条件与短促功能。缺少任一项时硬失败；具备证据的超短镜仍进入人工审阅摘要，连续超短镜段另行标记，但不按数量、比例或段长自动失败。
...
- dialogue edit plan 漏句、重复、乱序、逐字片段不完整、与 topology/正式画面不一致，用无新可见发展的 O.S. 留镜吞掉换人对白，或用 ID／镜号／复述台词／固定尾句伪造差异；
- 对白留镜的可见发展未在正式 blocking、performance、focus、movement、effect 或 state update 中实现；
- timing blocks 未覆盖 shot_flow、正时间分量没有对应 owner、活动事件与 hold 同块、hold 无独立 flow 证据、并行／顺序公式错误、对白重算不一致或正式镜长不等于时间块总和；
