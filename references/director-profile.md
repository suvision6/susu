...
导演 profile 不直接规定每镜的【角度，景别，运镜】，但会影响它们的组合倾向：

- `camera_energy=static` 时避免无动机运动，但必要时仍可用缓慢推/拉/摇。
- `camera_energy=responsive` 时摄影机可随动作、视线、声音变化移动，但每一下运动仍需具体理由。
- `visual_distance=intimate` 时倾向更近的景别，但不强制所有镜头为特写。
- `space_strategy=establish_then_enter` 时首镜更可能先交代空间关系。
- `space_strategy=embedded_reveal` 时空间在动作中逐步显露，允许延迟建立全景。

最终每镜的摄影三要素由镜头自身的叙事需求和 [shot-design.md](shot-design.md) 确定，不能用 profile 替代具体设计。
