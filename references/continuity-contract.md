...
只有镜头关系存在越轴、方向或动作接续风险时，才在场景 `axes[]` 登记可复核的轴线，并在相关镜头写 `continuity`。
...
发生真实状态变化时，`continuity_update` 包含：
...
- 后镜开场继承更新后的状态，不得覆盖历史。
- character 类型更新的精确 `field + from + to` 必须自然写入同一段 `【画面内容】`：position 写位移、facing 写朝向变化、eyeline 写视线转移、presence 写进出画，但不另起站位标签。不得因为同镜其他人物发生变化而给未变化人物补写“转向”。
