...
本文件是输入边界、原文锁、source span、稳定内部 ID、Beat、受保护 fact、导演分析隔离与来源叙事顺序的唯一规则源。
...
`performers[]` 只在人物归属无法从 fact 本身判断时使用。旧合同中的 `presentation_requirement`、`shot_isolation`、`isolation_reason` 与 `isolation_group_id` 可作为兼容字段读取，但 2.4.3 不再默认生成，也不以它们反推镜头数。
...
- dialogue fact 的 `text` 是逐字对白正文，不含角色名前缀、冒号或表演说明。
- 同时登记 `speaker` 与 `delivery: onscreen | offscreen | vo`；说话者必须来自当前来源场景，不要求另建完整人物台账来证明。
- 同一句对白不得因逗号、顿号、冒号、分号或人工换行拆成多个 fact 或相邻镜头。
- 对白拆镜不由标点或固定正反打决定，而由发言权、观看对象、声音位置、空间关系、节奏和剪辑收益决定；具体规则以 [dialogue-staging.md](dialogue-staging.md) 为准。
- 镜头 `dialogue[*].fact_id` 必须指向 dialogue fact，`text` 与其逐字相等。
- 不得把情绪意图、动作说明或 `speaker + ：` 混入对白正文。
- 对白可以与动作和表演并行发生；时长归属由 `shot-design.md` 决定。
...
- 旧版 `coverage_evidence[]` 可作为兼容输入继续校验，但 2.4.3 不再要求每个 fact 填写路径和逐字 quote。

## 10. Gate 影响

- Gate 1 digest 覆盖 source、实际展示的源分析、风格材料与 `director_profile`。
- Gate 2 digest 继承 Gate 1 digest，并覆盖实际展示的场级导演策略与完整 `shot_plan`。
