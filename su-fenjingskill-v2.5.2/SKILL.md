...
description: 将完整剧本、未编号剧本段落，或边界明确的连续场景片段转换为生产可执行的六列导演分镜和 shot-data/2.5.2 结构化交付。用于锁源、识别来源声音与屏幕事件、以切／留／镜内重构建立观看路径、完成三候选导演风格确认和 DOP 镜头设计，并落实轴线、视线、表演、阶段时长与必要连续性；不用于视频 Prompt、模型配置或合镜分组。
---
