...
正式身份固定为 `contract_name=shot-data`、`contract_version=2.5.2`、`source_skill=su-fenjingskill`、`source_skill_version=2.5.2`。
...
其中 `director_style_options`、`selected_style_option_id`、`emotion_arcs` 与 `performance_chains` 为按需字段。2.5.2 新生成不为填空而输出空分析结构。
...
`coverage_evidence[]` 是兼容字段。存在时仍验证路径和 quote 的真实性，但 2.5.2 不要求逐 fact 重复举证。复杂画面中的关键信息可用自然语言 `presentation_note` 指明观看重点。
...
旧版 `execution_passages[]` 只能作为迁移来源，不能代替 2.5.2 交付中的 `execution_text`，也不得与其并列成为第二套权威正文。
...
以下片段只说明 2.5.2 的规划关系，不是可直接构建的完整 draft：
