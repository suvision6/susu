...
`performers[]` 只在人物归属无法从 fact 本身判断时使用。旧合同中的 `presentation_requirement`、`shot_isolation`、`isolation_reason` 与 `isolation_group_id` 可作为兼容字段读取，但 2.4.4 不再默认生成，也不以它们反推镜头数。
...
- 旧版 `coverage_evidence[]` 可作为兼容输入继续校验，但 2.4.4 不要求每个 fact 填写路径和逐字 quote。
