#!/usr/bin/env node
/** su-fenjingskill 3.3.3: read authored Markdown; project, never rewrite shots. */
...
export function parseMarkdown(markdown) {
  ...
  if (value('Skill 版本：') !== VERSION) fail('请使用3.3.3主稿格式。', i - 1);
...
export async function exportFiles(input, outputDir, prefix, options = {}) {
  ...
  try {
    ...
    return { version: VERSION, shots: data.rows.length, estimated_seconds: data.total, markdown: mdPath, excel: xlsxPath, backup, staging, physical_shot_rows: plans[0].rows.length - plans[0].header, warnings };
  } finally {
    // Preserve staging for recovery and inspection; never recursively delete user paths.
  }
}
