#!/usr/bin/env python3
"""Create non-overwriting 3.1.4 review drafts from valid 3.1.3 inputs."""
...
SCRIPT_INTERFACE_REASON = "Explicit non-overwriting 3.1.3 to 3.1.4 draft migration."
ZERO_HASH = "0" * 64
...
def build_drafts(
    ...
    shot_draft["contract_version"] = "3.1.4"
    shot_draft["source_skill_version"] = "3.1.4"
    ...
    workspace_draft["workspace_contract"] = "director-workspace/3.1.4"
    ...
    gate_one.update({
        ...
        "note": "3.1.3 to 3.1.4 migration requires renewed Gate 1",
    })
    ...
    report = {
        "migration": "director-shot-data/3.1.3 -> director-shot-data/3.1.4",
        "workspace_migration": "director-workspace/3.1.3 -> director-workspace/3.1.4",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="非覆盖迁移 3.1.3 到 3.1.4 审阅草稿。")
...
def main(argv: Optional[Iterable[str]] = None) -> int:
    ...
    try:
        ...
        paths = {
            "shot_data_draft": args.output_dir / f"{slug}-shot-data-3.1.4-draft.json",
            "workspace_draft": args.output_dir / f"{slug}-director-workspace-3.1.4-draft.json",
