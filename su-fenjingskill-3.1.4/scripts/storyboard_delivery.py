#!/usr/bin/env python3
"""Formal director-shot-data/3.1.4 backend shipped by su-fenjingskill 3.1.4.
...
def _validate_timing_plan(
    ...
    for index, block in enumerate(blocks):
        ...
        if "performance" in owners and values["reaction_seconds"] <= 0:
...
def _combined_report(
    ...
    *,
    ...
    if workspace_missing:
        errors.append(issue("WORKSPACE_REQUIRED", "workspace", "3.1.4 正式 validate/build/export 必须提供 director-workspace/3.1.4。"))
...
def build_outputs(
    ...
    *,
    ...
    ``strict`` is retained for call-site compatibility; 3.1.4 is always fail-closed.
    """
    del strict
    ...
    if output_dir.exists() and (not output_dir.is_dir() or any(output_dir.iterdir())):
        blocking = issue(
            ...
            "output_dir",
            "3.1.4 正式 build-all 只允许不存在或为空的输出目录；拒绝覆盖任何已有文件。",
        )
        failed = copy.deepcopy(report)
...
def cmd_build(args: argparse.Namespace) -> int:
    if args.output_dir.exists() and (not args.output_dir.is_dir() or any(args.output_dir.iterdir())):
        sys.stderr.write("FAIL: 3.1.4 正式 build-all 输出目录必须不存在或为空；未修改任何已有文件。\n")
        return 1
    try:
        data = load_json(args.input)
        workspace = load_json(args.workspace) if args.workspace else None
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="su-fenjingskill 3.1.4 后端：Gate 0、对白观看计划、结构化时长、来源保护与原子四文件交付。"
    )
    ...
    validate_parser = subparsers.add_parser("validate", help="组合校验 3.1.4 workspace 与正式 shot data。")
    validate_parser.add_argument("--input", type=Path, required=True, help="输入 JSON 文件。")
    validate_parser.add_argument("--workspace", type=Path, help="director-workspace/3.1.4；正式 readiness 必需。")
    ...
    for command in ("build-all", "build"):
        ...
        build_parser_.add_argument("--workspace", type=Path, required=True, help="director-workspace/3.1.4。")
