# Migration Guide｜3.1.3 → 3.1.4

su-fenjingskill 3.1.4 使用 `director-shot-data/3.1.4` 与 `director-workspace/3.1.4`。3.1.3 已发布文件保持只读；禁止就地改版本号或把旧 READY validation 当成 3.1.4 证据。

## 3.1.3 → 3.1.4 显式迁移

```bash
python -m scripts.migrate_contract_3_1_3_to_3_1_4 \
  --shot-data <historical-3.1.3-shot-data.json> \
  --workspace <historical-workspace-3.1.3.json> \
  --output-dir <new-empty-directory>
```

迁移器保留来源、场景、镜头、既有拓扑、shot_flow 和结构化执行事实，但不猜：

- 节奏预设、目标总时长、画幅、方向、`su-dialogue-pace/1.0` 区间／选值或 pace overrides；
- 每句对白该切、该留还是镜内重构；
- 新 frame axis 与画幅应用；
- timing blocks、pace_ref 与重算镜长。

上述字段以 schema-valid unresolved 状态保存，Gate 0、Gate 1、Gate 2 与 Alignment 全部失效。完成 Gate 0、dialogue edit plan、画幅执行和 timing plan 后，才能重新确认后续阶段并进入正式构建。
