...
DIRECTING_PLAN_REQUIRED_KEYS = {"scene_objective", "progression", "pov_flow"}
...
def render_timing_note(shot: dict[str, Any]) -> str:
    action, dialogue, asynchronous, hold = timing_components(shot)
    total = max(action, dialogue) + asynchronous + hold
    return (
        f"[时长估算]同步动作{action}秒；同步台词{dialogue}秒；"
        f"非同步动作{asynchronous}秒；情绪留白{hold}秒；"
        f"前两项取 max 后再加后两项，共{total}秒。"
    )


def extract_execution_reminder(value: Any) -> str:
    note = clean_text(value)
    if note.startswith("[时长估算]"):
        remainder = note.split("。", 1)[1].strip() if "。" in note else ""
        if "[执行提醒]" in remainder:
            before, after = remainder.split("[执行提醒]", 1)
            return "".join(
                part.strip()
        ...
        return remainder
    if "[执行提醒]" in note:
        return note.split("[执行提醒]", 1)[1].strip()
    return note


def canonicalize_shot_notes(shot: dict[str, Any]) -> str:
    reminder = extract_execution_reminder(shot.get("notes"))
    note = render_timing_note(shot)
    if reminder:
        note += f"[执行提醒]{reminder}"
    return note
...
def render_dialogue(item: dict[str, Any]) -> str:
    ...
    delivery = clean_text(item.get("delivery"))
    addressee = clean_text(item.get("addressee"))
    if delivery == "vo":
        lead = f"{speaker}VO"
    elif delivery == "offscreen":
        lead = f"{speaker}画外"
    elif addressee:
...
def framing_display(camera: dict[str, Any]) -> str:
    ...
    prefixes = {
        ...
        "two_shot": "双人",
        "multi_shot": "多人",
        "continuous_reframe": "连续重构",
        "subjective": "主观",
        "insert": "插入",
        "environment": "环境",
    }
    prefix = prefixes.get(framing_mode, "")
    if not prefix or shot_size.startswith(prefix):
        return shot_size
    return f"{prefix}{shot_size}"
...
def validate_confirmations(data: dict[str, Any], result: ValidationResult) -> None:
    ...
    for key in sorted(actual - expected_keys):
        result.error("CONFIRMATION_UNKNOWN", f"$.confirmations.{key}", "不是 2.4.3 人工 Gate。")
...
def validate_gate_1_material(data: dict[str, Any], result: ValidationResult) -> None:
    ...
    for index, option in enumerate(options):
        ...
        require_nonempty_string(option.get("label"), path=f"{path}.label", result=result)
        require_nonempty_string(option.get("rationale"), path=f"{path}.rationale", result=result)
...
def validate_directing_plan(value: Any, *, path: str, result: ValidationResult) -> None:
    if not isinstance(value, dict):
        result.error(
    ...
    if incomplete:
        result.error(
            ...
            path,
            "整场规划必须说明目标、推进和视点；入口、出口、节奏、受保护过程与视觉转折只在场景需要时填写。",
        )
    ...
    for index, beat in enumerate(beats):
        ...
        for fact_index, fact in enumerate(facts):
            ...
            if fact_type == "dialogue":
                ...
                if fact.get("delivery") not in DIALOGUE_DELIVERIES:
                    result.error("DIALOGUE_DELIVERY", f"{fact_path}.delivery", "对白 delivery 无效。")
...
def dialogue_facts_for_plan_unit(
    ...
    unit_ranges = span_coordinates(unit.get("source_spans"), locked_text)
    output: list[dict[str, Any]] = []
    for beat_id in as_list(unit.get("beat_ids")):
        for fact in as_list(beat_lookup.get(str(beat_id), {}).get("facts")):
...
def validate_shot_plan(
    ...
    *,
    ...
    for index, unit in enumerate(units):
        ...
        if unit.get("shot_form") is not None and unit.get("shot_form") not in SHOT_FORMS:
            result.error(
                "SHOT_FORM",
                f"{path}.shot_form",
                "普通镜头省略 shot_form；只有明确采用长镜头时写 long_take。",
            )
        if not is_json_integer(unit.get("estimated_duration_seconds"), 1):
            result.error(
                "PLAN_UNIT_DURATION",
        ...
        validate_dialogue_design(
            ...
            dialogue_facts=dialogue_facts_for_plan_unit(unit, beat_lookup, locked_text),
    ...
    for index, edit_point in enumerate(edit_points):
        ...
        expected_after = units[index].get("plan_unit_id") if index < len(units) and isinstance(units[index], dict) else None
        expected_before = (
            units[index + 1].get("plan_unit_id")
    ...
    edit_count = max(planned_count - 1, 0)
    durations = [
...
def validate_camera(
    ...
    *,
    ...
    if scene_characters:
        for character in primary_subjects | foreground:
    ...
    if any(token in movement for token in ("固定", "静止", "锁定")) and any(
    ...
    if scene_characters is not None:
        for character in primary_subjects | foreground:
            if character not in scene_characters:
                result.error("CAMERA_SUBJECT", path, f"摄影机人物 `{character}` 不在场景台账。")
    if primary_subjects & foreground:
        result.error("CAMERA_SUBJECT", path, "同一人物不能同时是主拍主体和前景肩背。")
    ...
    static_motion = any(token in movement for token in ("固定", "静止", "锁定"))
    ...
    shot_size = clean_text(camera.get("shot_size"))
    if any(token in shot_size for token in ("远景", "全景")) and any(
        token in position for token in ("贴近", "近处", "近距离")
    ):
        result.warn(
            "CAMERA_SCALE_POSITION_TENSION",
            f"{path}.position",
            "远景／全景与近距离机位并置，请确认是否有明确的宽角近距构图理由。",
        )
    if any(token in shot_size for token in ("特写", "大特写")) and any(
        token in position for token in ("远处", "远距离")
    ):
        result.warn(
            "CAMERA_SCALE_POSITION_TENSION",
            f"{path}.position",
            "特写与远距离机位并置，请确认焦段和主体可读性。",
        )
    ...
    *,
    ...
    for index, item in enumerate(dialogue):
        ...
        if not fact or fact.get("type") != "dialogue":
            ...
            if item.get("delivery") != fact.get("delivery"):
                result.error("DIALOGUE_DELIVERY", f"{item_path}.delivery", "delivery 与 fact 不一致。")
        ...
        if delivery not in DIALOGUE_DELIVERIES:
            result.error("DIALOGUE_DELIVERY", f"{item_path}.delivery", "delivery 无效。")
        elif (
            delivery == "onscreen"
        ...
        if timing and duration_block_ids and timing not in duration_block_ids:
            result.error("DIALOGUE_TIMING", f"{item_path}.timing", "必须引用本镜 duration block_id。")
    ...
    *,
    ...
    for item in dialogue:
        ...
        if presentation is None:
            ...
            continue
        if delivery == "onscreen" and presentation in {"offscreen", "vo"}:
            result.error(
                "DIALOGUE_PLAN_CAMERA_MISMATCH",
                path,
        ...
        if delivery == "offscreen" and presentation != "offscreen":
            result.error("DIALOGUE_PLAN_CAMERA_MISMATCH", path, "offscreen 对白必须标记 offscreen。")
        if delivery == "vo" and presentation != "vo":
            result.error("DIALOGUE_PLAN_CAMERA_MISMATCH", path, "VO 对白必须标记 vo。")
...
def validate_execution_text(
    ...
    *,
    ...
    for key, label in (
        ...
        camera_value = normalize_execution_text(camera.get(key))
        if camera_value and camera_value not in normalized_content:
            result.error(
                "EXECUTION_CAMERA_RELATION",
                f"{path}.execution_text",
                f"【画面内容】必须自然写入 camera.{key} 对应的{label}。",
            )
    for phrase in PLACEHOLDER_PHRASES:
...
def validate_shots(
    ...
    for index, shot in enumerate(shots):
        ...
        validate_duration_blocks(shot, path, result)
        validate_camera(
            shot.get("camera"),
            f"{path}.camera",
            result,
        ...
        validate_dialogue(
            ...
            scene_characters=scene_characters,
            visible_characters=visible_characters,
            duration_block_ids=duration_block_ids(shot),
            result=result,
        )
        ...
        if not re.match(
            ...
            result.error(
                "RENDERED_DESCRIPTION_FORMAT",
                f"{path}.rendered_shot_description",
                "第五列必须为统一摄影三要素加一个【画面内容】。",
            )
        ...
        for update_index, item in enumerate(as_list(shot.get("continuity_updates"))):
            ...
            required_values = (
                ...
                clean_text(item.get("from")),
                clean_text(item.get("to")),
            )
            if any(
        ...
        expected_timing_note = render_timing_note(shot)
        reminder = ""
        if not note.startswith(expected_timing_note):
            result.error(
                "TIMING_NOTE",
                f"{path}.notes",
                "备注必须以由标准时间块确定性生成的时长估算开头。",
            )
        else:
            suffix = note[len(expected_timing_note) :]
            if suffix:
                if not suffix.startswith("[执行提醒]"):
                    result.error(
                        "TIMING_NOTE",
                        f"{path}.notes",
                        "时长估算后只能追加一个【执行提醒】语义段。",
                    )
                else:
                    reminder = clean_text(suffix[len("[执行提醒]") :])
                    if not reminder:
                        result.error(
                            "SHOT_NOTE_EMPTY",
                            f"{path}.notes",
                            "存在执行提醒标记时必须写出本镜独有的提醒。",
                        )
        normalized_reminder = normalize_execution_text(reminder)
        if reminder in {"无", "同上", "无备注", "同前镜", "无需备注"}:
            result.error(
                "SHOT_NOTE_EMPTY",
                f"{path}.notes",
                "执行提醒不得使用空值式占位语。",
        ...
        if (
            normalized_reminder
            and normalized_reminder in normalize_execution_text(expected_description)
        ):
            result.error(
                "SHOT_NOTE_DUPLICATE",
                f"{path}.notes",
                "执行提醒不得复制第五列；应补充本镜独有的制作或连续性提醒。",
            )
        if VISIBLE_MACHINE_STATE_PATTERN.search(note):
            result.error(
                "VISIBLE_MACHINE_STATE",
                f"{path}.notes",
                "备注不得暴露内部规划、轴线侧别或机器状态 ID。",
            )
    ...
    for index, shot in enumerate(shots[:-1]):
        if isinstance(shot, dict) and as_dict(shot.get("transition_to_next")).get("type") == "scene_end":
            result.error(
                "NONFINAL_TRANSITION",
    ...
    *,
    ...
    for shot_index, shot in enumerate(shots):
        ...
        passage_texts = [
            ...
            for passage in as_list(shot.get("execution_passages"))
            if isinstance(passage, dict) and clean_text(passage.get("text"))
        ]
        for left_index, left in enumerate(passage_texts):
            left_tokens = set(re.findall(r"[\u4e00-\u9fff]{2,}|[A-Za-z0-9]+", left))
            for right in passage_texts[left_index + 1 :]:
                right_tokens = set(re.findall(r"[\u4e00-\u9fff]{2,}|[A-Za-z0-9]+", right))
                union = left_tokens | right_tokens
                if False and union and len(left_tokens & right_tokens) / len(union) >= 0.75:
                    result.warn(
                        "EXECUTION_SEMANTIC_OVERLAP",
                        f"$.shots[{shot_index}].execution_passages",
                        "同一镜内执行段落语义高度重叠，请检查动作、反应与台词是否被重复书写。",
                    )
                    break

    if False and len(shots) >= 8:
        single_line_count = 0
        for shot in shots:
            source_text = span_source_text(shot.get("source_spans"), locked_text)
            nonblank_lines = [line for line in source_text.splitlines() if line.strip()]
            if len(nonblank_lines) <= 1:
                single_line_count += 1
        ratio = single_line_count / len(shots)
        if ratio >= 0.60:
            result.warn(
                "MECHANICAL_SPLIT_DENSITY",
                "$.shots",
                f"{single_line_count}/{len(shots)} 镜只承载一行或更少原文，疑似按行机械拆镜。",
            )
        ...
        for shot in shots:
            ...
            dialogue_facts = [fact for fact in covered if fact.get("type") == "dialogue"]
            if dialogue_facts:
                dialogue_shots += 1
                if len(covered) == len(dialogue_facts):
                    dialogue_only += 1
        if dialogue_shots and dialogue_only / dialogue_shots >= 0.60:
            result.warn(
                "DIALOGUE_ISOLATION_DENSITY",
                "$.shots",
                f"{dialogue_only}/{dialogue_shots} 个对白镜只承载对白，需检查是否把单句或短反应机械独立成镜。",
            )
        ...
        for field_name in ("composition", "movement"):
            values = [
                clean_text(as_dict(shot.get("camera")).get(field_name))
                for shot in shots
            ]
            ...
            if frequencies:
                repeated_value, count = max(frequencies.items(), key=lambda item: item[1])
                if count / len(shots) >= 0.50:
                    result.warn(
                        "CAMERA_TEMPLATE_REPETITION",
                        f"$.shots[*].camera.{field_name}",
                        f"{count}/{len(shots)} 镜重复同一{field_name}模板：{repeated_value}",
                    )
...
def atomic_write_delivery(
    ...
    if any((output_dir / name).exists() for name in legacy_names):
        raise ValueError("输出目录含旧版固定文件名；2.4.3 必须使用带剧本前缀的新目录。")
...
def command_build(args: argparse.Namespace) -> int:
    ...
    output = {
        ...
        "content_hash": report["source_content_hash"],
