...
正式身份固定为 `contract_name=shot-data`、`contract_version=2.5.3`、`source_skill=su-fenjingskill`、`source_skill_version=2.5.5`。

顶层字段、按需字段、基础类型和未知字段拒绝策略只由 [shot-data.schema.json](shot-data.schema.json) 定义。2.5.5 Skill 仍生成 `shot-data/2.5.3`，不为填空而输出空分析结构，也不向公开合同增加字段。
