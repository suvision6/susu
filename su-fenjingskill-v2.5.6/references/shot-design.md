...
正文只读取唯一 `execution_text`。旧版 `execution_passages[]` 只能作为迁移材料，不是 2.5.3 权威正文。正式新建正文不再拆分【镜头调度】【人物表演与声音】【镜头结束】三段，而是写成一个连续的【画面内容】段落，顺序为：
...
`supports[]` 只允许 `continuous_action | performance_development | spatial_progression | blocking_proof | real_time_tension | monologue_delivery | testimony_statement`。保护范围只能引用当前单元事件。

- `monologue_delivery`：角色向观众或对象进行长段独白，完整性优先。
- `testimony_statement`：法庭、审讯、听证等正式陈述，需要连续权威感。

使用 `monologue_delivery` 或 `testimony_statement` 时，放宽普通剧情镜 10 秒限制，但必须满足：
1. `dialogue_design.speaker_sequence[]` 只含一个说话者。
2. `protected_event_ids` 覆盖整个独白或陈述事件。
3. `long_take_design.reason` 说明为何连续拍摄优于剪辑。
4. 独白/陈述结束后若有他人反应，反应必须作为新事件单独处理，不得被长镜保护范围吞并。
