...
def build_execution_text(shot: dict, facts: list[dict], environment: str) -> str:
    ...
    for f in facts:
        if f.get("type") == "dialogue":
            line = f'{f["speaker"]}：{f["text"]}'
        else:
            line = f["text"]
        if line not in fact_lines:
            fact_lines.append(line)
    behavior = shot.get("performance", {}).get("emotion_intent", "")
    position = camera["position"]
    logic = camera["logic"]
    execution_position = re.sub(
        r"(?:朝向|面向|看向|望向|朝)[^，；。]*$",
        "",
        position,
    ).rstrip("，；。 ")
    camera_sentence = (
        f"摄影机{logic}"
        if delivery.normalize_execution_text(execution_position)
        in delivery.normalize_execution_text(logic)
        else f"摄影机位于{execution_position or position}，{logic}"
    )
    composition = (
        camera["composition"]
    ...
    fact_norms = [
        delivery.normalize_execution_text(fact.get("text"))
        for fact in facts
        if delivery.normalize_execution_text(fact.get("text"))
    ]
    behavior_norm = delivery.normalize_execution_text(behavior)
    if behavior and not any(
        fact_norm in behavior_norm or behavior_norm in fact_norm
        for fact_norm in fact_norms
    ):
        parts.append(f"{behavior}。")
    parts.append(f"最后停在{'；'.join(shot.get('end_state', []))}。")
    return "".join(parts)
