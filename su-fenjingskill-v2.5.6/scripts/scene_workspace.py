...
WORKSPACE_CONTRACT = "shot-data-scene-workspace/2"
LEGACY_WORKSPACE_CONTRACT = "shot-data-scene-workspace/1"
...
def merge_scene(data: dict[str, Any], workspace: dict[str, Any]) -> dict[str, Any]:
    if workspace.get("workspace_contract") == LEGACY_WORKSPACE_CONTRACT:
        raise ValueError(
            "scene workspace /1 不含场景基线摘要；请从当前 draft 重新 extract。"
        )
