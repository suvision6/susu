...
- 当前 Skill 版本为 `2.5.6`，正式数据合同保持 `shot-data/2.5.3`；Gate 2 规则修订号仍为 `2.5.4-binding-integrity-r1`。本次修订只排除 `SCENE` / `CUT TO` 等剧本制作标签造成的相邻双语台词误报，不改变语言角色合同、`shot-data/2.5.3` 或 Gate 2 视觉规划语义。
