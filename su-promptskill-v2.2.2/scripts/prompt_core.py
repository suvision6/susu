...
def tag_present(tag: str, text: str) -> bool:
    """Compare complete identifiers: @图片1 must not match @图片10.

    Chinese prose may immediately follow an identifier (e.g. 作为首帧).
    This is lexical matching, not reference observation or upload verification.
    """
    return re.search(r'(?<![A-Za-z0-9_.-])' + re.escape(tag) +
                     r'(?![A-Za-z0-9_.-])', text) is not None


def accessible_assets(cfg: dict, tags: dict, context: dict, uid: str, add):
    """Resolve an explicit per-request declaration; never infer actual access.

    Missing means legacy inventory behaviour. [] means explicitly no access.
    Invalid declarations are errors and do not fall back to a permissive scope.
    """
    if 'accessible_asset_tags' not in cfg:
        available = {t for t, a in tags.items() if a.get('available', False)}
        return (available if context.get('inventory_complete') else None,
                {'basis': 'legacy_inventory' if context.get('inventory_complete') else 'unknown',
                 'provider_access': 'not_verified_by_script'})
    values = cfg['accessible_asset_tags']
    valid = True
    if not isinstance(values, list) or any(
            not isinstance(t, str) or not t or t != t.strip() for t in values):
        add('ACCESSIBLE_ASSET_TYPE', 'accessible_asset_tags必须是原样非空tag组成的数组；未知时省略，不用null或空数组代替未知。', uid)
        return set(), {'basis': 'invalid_declaration', 'provider_access': 'not_verified_by_script'}
    if len(values) != len(set(values)):
        add('ACCESSIBLE_ASSET_DUPLICATE', '本次可访问素材tag重复。', uid)
        valid = False
    for t in values:
        if t not in tags:
            add('ACCESSIBLE_ASSET_UNKNOWN', '本次可访问素材不在库存：' + t, uid)
            valid = False
        elif not tags[t].get('available', False):
            add('ACCESSIBLE_ASSET_UNAVAILABLE', '本次可访问范围含不可用素材：' + t, uid)
            valid = False
    return set(values), {'basis': 'explicit_declaration' if valid else 'invalid_declaration',
                         'declared_tags': list(values),
                         'provider_access': 'not_verified_by_script'}
...
def assess(master: dict, source: dict | None, context: dict | None=None) -> dict:
    ...
    tags={}
    for a in assets:
        if (not isinstance(a,dict) or not isinstance(a.get('tag'),str) or not a['tag'] or a['tag'] != a['tag'].strip()):raise FormatError('素材记录需原样非空字符串tag。')
    ...
    unit_configs = context.get('units', {})
    if not isinstance(unit_configs, dict):raise FormatError('units必须是按单元ID索引的对象。')
    ...
    for u in master['units']:
        uid=u['id'];p=u['prompt_text'];cfg=unit_configs.get(uid,{})
        if not isinstance(cfg,dict):raise FormatError(uid+'的单元配置必须是对象。')
        accessible, access_report = accessible_assets(cfg, tags, context, uid, add)
        ...
        standard_tags=set(TAG.findall(p)); known_occurrences={t for t in tags if tag_present(t,p)}
        ...
        if not isinstance(assignments,list) or any(not isinstance(r,dict) or not isinstance(r.get('tag'),str) or not r['tag'] for r in assignments):
            add('ASSIGNMENT_TYPE','references必须是含明确tag的对象数组。',uid)
            assignments=[]
        active={r['tag'] for r in assignments}
        if assignments:
            for r in assignments:
                ...
                if not tag_present(t,p):add('ASSIGNMENT_NOT_IN_PROMPT','已采用素材没有进入实际正文：'+t,uid)
                ...
                elif used_section and not tag_present(t,used_section):add('ASSET_ROLE_NOT_VISIBLE','职责未进入对应区块：'+t,uid)
        ...
        if 'accessible_asset_tags' in cfg:
            for t in sorted(active - accessible):
                add('ASSIGNMENT_NOT_ACCESSIBLE','已采用素材不在本次明确可访问范围：'+t,uid)
            for t in sorted((standard_tags | known_occurrences) - accessible):
                add('ASSET_NOT_ACCESSIBLE','正文引用本次未提供的素材：'+t,uid)
        if accessible is not None:
            expected=accessible-active
            listed={t for t in tags if tag_present(t,unused_section)} | set(TAG.findall(unused_section))
            if expected!=listed:add('UNUSED_ASSET_SET','未采用素材不等于本次适用素材范围减采用集合；缺新声明时沿用旧库存规则。',uid)
        for t in active:
            if tag_present(t,unused_section):add('ASSET_ACTIVE_AND_UNUSED','同一素材同时采用和排除：'+t,uid)
        ...
        per_unit.append({'id':uid,'source_refs':refs,'mode':mode,'duration_exact':str(total) if total is not None else None,'timeline':timeline,'asset_access':access_report})
    if source:
