...
读取导演卡后，按 [director-profile.md](director-profile.md) 把卡内“Profile 默认建议”编译为本场专属 `director_profile`。所有卡片共同遵守：入口从当前来源的空间、人物关系、动作、声音或主观锚点推导；对白按发言权与画面所有权处理，只有特殊倾听者留镜或延迟反打才登记 `dialogue_design`；不复刻具体电影镜头，只把风格倾向改写为当前场景策略。卡片只保留与该导演有区分度的时间、摄影机、空间、表演、收益、风险和 profile 内容。
