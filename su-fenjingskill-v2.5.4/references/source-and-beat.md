...
`performers[]` 只在人物归属无法从 fact 本身判断时使用。旧合同中的 `presentation_requirement`、`shot_isolation`、`isolation_reason` 与 `isolation_group_id` 在 v2.5.3 中仍可作为兼容字段读取，但**不得再默认生成**，也不以它们反推镜头数。v2.5.3 是最后一个兼容读取这些字段的版本；v2.6 将完全移除读取支持。若旧数据存在 `shot_isolation=director_required`，Skill 必须把它转换为明确的 `presentation_note` 或导演分析，而不是依赖旧字段反推镜头数。
...
- 旧版 `coverage_evidence[]` 可作为兼容输入继续校验，但 2.5.3 不要求每个 fact 填写路径和逐字 quote。
