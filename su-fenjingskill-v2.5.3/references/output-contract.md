...
正式身份固定为 `contract_name=shot-data`、`contract_version=2.5.3`、`source_skill=su-fenjingskill`、`source_skill_version=2.5.3`。
...
其中 `director_style_options`、`selected_style_option_id`、`emotion_arcs` 与 `performance_chains` 为按需字段。2.5.3 新生成不为填空而输出空分析结构。
...
`gate_2_rule_revision` 固定为 `2.5.3-binding-integrity-r1`。`scene_plans` 是 `scenes[]` 的派生字段；原子事件字段、`non_cut_basis`、长镜保护范围、节奏指标、`shot_plan`、DOP 字段、`visual_design` 与 `director_readiness` 全部进入 Gate 2 digest。计算前统一规范化 `locked_text`、源 hash 与全部 span hash。任何相关内容或规则修订号变化都会使旧 Gate 2 确认失效。
...
终稿 `camera` 的执行字段包含 `shot_size`、`angle`、`position`、`logic`、`composition` 与 `movement`；绑定字段还必须包含 `viewpoint_owner`、`primary_subjects[]`、`secondary_subjects[]`、`perspective_intent`、`focus_plan`、`spatial_strategy`、`movement_plan`、`start_frame`、`end_frame` 与 `motivation`。除 `logic`、纯运镜文字和按需 `framing_mode`／`foreground_characters[]` 外，绑定字段必须与对应 Gate 2 `visual_plan` 完全一致。
