...
def disallowed_generated_tokens(
    ...
    *,
    ...
    standard production term, appear verbatim in the locked source, or be short
    enough to be considered an embedded foreign token.
    """
    ...
    for token in script_tokens(text):
        ...
        if token in locked_text:
            continue
        if _is_embedded_foreign(token, primary_family=None):
            continue
