...
```json
{
  ...
  "source": {
    "file_sha256": "",
    "content_hash": "",
    "skill_version": "上游 source_skill_version 原值",
...
标注器只接受 `release_ready=true` 且 `source_file_sha256` 与当前输入 JSON 原始文件字节一致的 page-map。外围标签固定渲染为 `display_label｜camera_label`，例如 `C005-A｜过肩近景`。
...
- fallback warning。
- 每格 `display_label` 与 `camera_label`。
