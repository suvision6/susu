---
name: su-image9
description: 从结构正常、具备可画镜头能力的 shot_data.json 只读派生 Image2 3x3 黑白石墨电影分镜提示词、九格原图与带镜号和景别的外围标注 PNG。按输入能力与数据形状适配，不要求上游 Skill 名称、版本、合同名、Gate、beats 或 hash 字段；以 panel_plan.json 为唯一机器事实源，严格保持源镜顺序、原构图、场景、现实层、人物、道具和连续性，绝不回写或反向要求上游。
---
...
它不拆剧本、不改镜头、不回写主表，也不得把本地执行问题回流为上游修改要求。

2.2.0 改为能力驱动的只读输入适配：

- 不把 `source_skill`、`source_skill_version`、`contract_name`、Gate、`beats`、`content_hash` 或锁源 hash 当作上游必填项。
- 只检查完成任务真正需要的能力：非空场景、唯一递增镜号、合法 scene 归属、可读机位/景别、可画调度或执行文本。
- 在本地深拷贝视图中计算来源摘要，并从 `shot_no`、`shot_order` 或 `SHxxx` 归一化镜号；从嵌套 `camera`、`staging`、`sound` 和 scene `space_map` 归一化稳定字段。
- 若旧 `shot-data` 合同完整，继续沿用其原生字段；若是 `director-shot-data` 或其他正常形状，则按能力适配。
- 适配、审计、派生与验证均不得修改来源对象或来源文件。
- 发现不兼容或语义冲突时只生成本地失败／审查报告；不得要求 `su-fenjingskill` 回写、重跑或改变合同。
- 外围标注固定显示 `镜号｜景别`；派生格显示 `Cxxx-A｜派生景别`。

2.2.0 相对 2.1.1 的主要升级：
...
- **图像生产骨架**：新增 `generate_panels.py`（默认 stub 后端，输出占位 PNG）与 `visual_qa.py`（机器几何/灰度检查 + 人工复核清单）。真实模型接入作为后续任务，不在 2.2.0 范围内。
...
- 检查九格页是否忠实继承源镜事实与连续性。
- 运行中文语义审计并报告上游不可画冲突（2.2.0）。
- 从单格 PNG 确定性拼版并标注（2.2.0）。

以下任务不使用本 Skill：

- 剧本拆 Beat、拆镜或修改导演镜头设计。
- 修改 `su-fenjingskill` 主表、Prompt、结构化数据或 Excel。
- 彩色概念图、写实剧照、漫画页或其他视觉风格。
- text-only 交付。
- 真实图像模型调用（2.2.0 仅完成骨架，模型接入为后续任务）。
...
正式入口不限制上游 Skill、版本或合同名，只拒绝无法安全完成任务的结构损坏物料：

- `scenes` 与 `shots` 必须为非空数组。
- 每个 scene 必须有唯一非空 `scene_id`。
- 每个镜头必须能从 `shot_no`、`shot_order` 或数字型 `shot_id` 得到唯一递增镜号，并归属已声明 scene。
- 每镜 `camera` 必须至少给出非空 `shot_size` 与 `angle`。
- 每镜必须具有可画的调度、可见主体或 `execution_text`。
- 来源文件 SHA-256 与本地规范化内容摘要由 `su-image9` 自行记录；上游不需要预先提供 hash。
- 缺失 Beat、事实 ID、Gate 或版本信息时，只在内部只读视图中确定性派生，不写回来源。

任何上游名称、版本号或合同名都不构成拒绝理由。只有完成本任务必需的可画字段缺失或互相冲突时，才在 `su-image9` 本地失败并停止；禁止要求上游补写指定版本字段。

语义审计在输入门禁之后、分页与派生之前运行。语义冲突不阻塞输入合同，但会在本地产物中生成 `F-SEMANTIC-CONFLICT` 审查项；用户未给出本地解释或替换输入前不进入 release-ready 状态，不得把审查要求回流给上游 Skill。
...
- `primary_focus`、`must_show`、`may_show`、`must_not_show`（2.2.0）。
- `render_delta`、`story_delta`、`camera_rationale`（2.2.0）。
...
`generate_panels.py` 读取 `panel_plan.json` 与 `final_image_prompts.compiled.md`，为每页每格生成单张 16:9 PNG。v2.2.0 默认使用 stub 后端输出占位灰度图；真实模型接入为后续任务。
...
- 单格生成目录 `PAGE-XX/PANEL-N.png` 与 `attempt_log.json`（2.2.0）。
- 可选 `visual_qa_report.json`（2.2.0）。
...
标签只读取 `page-map.json` 的 `display_label` 与 `camera_label`，正式格式为 `镜号｜景别`；标注前必须核对 page-map 为 2.1、`release_ready=true`，且其 source file SHA-256 与当前输入 JSON 文件一致。
...
- 不修改 `su-fenjingskill`，不向其回流修改、重跑或补字段要求。
- 不回写导演主表。
- 不让摄影术语反向改变源镜事实。
- 不让参考图改变剧情。
