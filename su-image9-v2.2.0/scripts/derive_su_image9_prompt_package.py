#!/usr/bin/env python3
"""Derive a deterministic su-image9 v2.2.0 package from usable shot data."""
...
from upstream_adapter import (
    ADAPTER_KEY,
    CANONICAL_UPSTREAM_SKILL,
    SUPPORTED_UPSTREAM_SKILLS,
    UpstreamAdapterError,
    adapt_upstream,
    adapted_content_hash,
)


VERSION = "2.2.0"
SCHEMA_VERSION = "2.1"
UPSTREAM_SKILL = CANONICAL_UPSTREAM_SKILL
PAGE_SIZE = 9
...
def canonical_data_hash(data: dict[str, Any]) -> str:
    adapted_hash = adapted_content_hash(data)
    if adapted_hash:
        return adapted_hash
...
def validate_source_contract(data: Any) -> list[str]:
    ...
    metadata = as_dict(data.get("metadata"))
    if metadata.get("skill_name") not in SUPPORTED_UPSTREAM_SKILLS:
        errors.append(f'metadata.skill_name must be "{UPSTREAM_SKILL}"; version is unrestricted')
                ...
                "panels": [
                    {
                        ...
                        "camera_label": annotation_camera_label(panel),
                    }
                    for panel in page["panels"]
                ],
            }
...
def annotation_camera_label(panel: dict[str, Any]) -> str:
    """Return a compact Chinese framing label for external annotation."""
    drawn = clean_text(panel.get("drawn_camera_tag"))
    derived_labels = (
        ("same-axis wider", "同轴更宽"),
        ("same-axis tighter", "同轴更紧"),
        ("three-quarter", "同侧三分之四"),
        ("profile", "同侧侧面"),
        ("high angle", "高机位"),
        ("low angle", "低机位"),
        ("over-shoulder", "过肩近景"),
        ("registered-prop insert", "道具特写"),
        ("speaker close-up", "说者近景"),
        ("listener reaction", "听者反应近景"),
        ("action start", "动作起点"),
        ("action process", "动作过程"),
        ("action end", "动作终点"),
        ("subjective-framing", "主观构图"),
    )
    lowered = drawn.lower()
    for token, label in derived_labels:
        if token in lowered:
            return label
    source = clean_text(panel.get("source_camera_tag"))
    parts = [clean_text(item) for item in re.split(r"[,，]", source) if clean_text(item)]
    if len(parts) >= 2:
        return parts[1]
    return source or "景别未标明"
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        data = adapt_upstream(load_source(args.shot_data))
    except UpstreamAdapterError as exc:
