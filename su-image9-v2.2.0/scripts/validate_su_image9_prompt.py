#!/usr/bin/env python3
"""Fail-closed validator/compiler for the su-image9 2.2.0 prompt package."""
...
# 2.2.0 extends the panel contract with narrative-driven visibility fields.
PANEL_KEYS = {
    "panel",
    "panel_kind",
    "source_shot",
...
def read_version(skill_dir: Path, audit: Audit) -> str:
    ...
    if version != SKILL_VERSION:
        audit.fail(
            "F-VERSION",
            "VERSION",
            "validator only implements the 2.2.0 contract",
...
def require_exact_keys(value: Any, keys: set[str], field_name: str, audit: Audit) -> bool:
    ...
    if actual != keys:
        audit.fail(
            "F-PLAN-SCHEMA",
            field_name,
            "object keys do not match the public 2.2.0 schema",
...
def validate_panel_plan(plan: dict[str, Any], shot_data: dict[str, Any], canon_hash: str, audit: Audit) -> None:
    ...
    if plan.get("skill") != SKILL_NAME or plan.get("version") != SKILL_VERSION or plan.get("schema_version") != SCHEMA_VERSION:
        audit.fail(
            "F-PLAN-SCHEMA",
            "panel_plan.identity",
            "plan identity does not match the 2.2.0 contract",
...
def validate_files(args: argparse.Namespace) -> tuple[int, dict[str, Any], str]:
    ...
    if shot_data is not None:
        try:
            shot_data = adapt_upstream(shot_data)
        except UpstreamAdapterError as exc:
            for message in exc.errors:
                audit.fail("F-SOURCE", "shot_data", message)
            shot_data = None
