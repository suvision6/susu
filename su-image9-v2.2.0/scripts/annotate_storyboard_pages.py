...
v2.2.0 supports two input modes:
...
from upstream_adapter import CANONICAL_UPSTREAM_SKILL, adapt_upstream, upstream_identity
...
def validate_page_map_contract(page_map: dict[str, Any], data_path: Path) -> None:
    ...
    if set(page_map) != expected_keys:
        raise ValueError("page-map top-level fields do not match the su-image9 2.2.0 contract")
...
def load_page_specs(page_map: dict[str, Any]) -> list[PageSpec]:
    ...
    for raw_page in pages:
        ...
        for raw_panel in raw_panels:
            ...
            camera_label = raw_panel.get("camera_label")
            if not isinstance(camera_label, str) or not camera_label.strip():
                raise ValueError(f"PAGE-{page_no:02d}/PANEL-{panel_no} camera_label is required")
            ...
            panels.append(PanelSpec(panel_no, label.strip(), camera_label.strip(), source_shot))
...
def detect_grid_boxes(image: Image.Image) -> dict[int, tuple[int, int, int, int]] | None:
    ...
    for row in range(3):
        for column in range(3):
            ...
            panel_no += 1
    widths = [right - left for left, _top, right, _bottom in boxes.values()]
    heights = [bottom - top for _left, top, _right, bottom in boxes.values()]
    if min(widths) < width * 0.20 or min(heights) < height * 0.18:
        return None
    if max(widths) > min(widths) * 1.20 or max(heights) > min(heights) * 1.20:
        return None
    return boxes
...
def fit_font_to_box(
    ...
    *,
    ...
    available = max(1, box[2] - box[0] - horizontal_padding * 2)
    candidate = font
    while candidate.size > minimum_size:
        text_box = draw.textbbox((0, 0), text, font=candidate)
        if text_box[2] - text_box[0] <= available:
            return candidate
...
def render_page(
    ...
    if mode == "per-panel":
        source = composite_page(source_path, page, target_width=1920, target_height=1080)
    ...
    for panel in page.panels:
        ...
        rendered_label = f"{panel.display_label}｜{panel.camera_label}"
        label_font = fit_font_to_box(draw, rendered_label, font, label_box)
        draw.text(
            centered_text(draw, rendered_label, label_font, label_box),
            rendered_label,
            fill="#111111",
            font=label_font,
        )
    ...
    return {
        ...
        "camera_labels": {str(panel.panel_no): panel.camera_label for panel in page.panels},
    }
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if upstream_identity(data) == CANONICAL_UPSTREAM_SKILL:
            data = adapt_upstream(data)
        ...
        with tempfile.TemporaryDirectory(
            ...
            for page in page_specs:
                if args.mode == "per-panel":
                    source_path = args.pages.resolve() / Path(page.source).stem
                    # source_path is the PAGE-XX directory containing PANEL-N.png
