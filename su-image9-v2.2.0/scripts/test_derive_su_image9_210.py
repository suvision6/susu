#!/usr/bin/env python3
"""Contract and end-to-end tests for the su-image9 v2.2.0 deriver."""
    ...
    for parent in (start, *start.parents):
        for candidate in (
            ...
            if (candidate / "storyboard_delivery.py").is_file() and (
                ...
                return candidate
    raise unittest.SkipTest("legacy su-fenjingskill-zh 2.4.2 fixture is unavailable")
...
class Derive212Tests(unittest.TestCase):
    ...
    def test_real_242_final_signoff_builds_release_ready_212_package(self) -> None:
        ...
        with temp:
            ...
            self.assertEqual("2.2.0", plan["version"])
    ...
    def test_validator_failure_publishes_only_the_report(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_name:
            ...
            bad_canon.write_text("<!-- canon-version: 2.2.0 -->\n", encoding="utf-8")
            process = subprocess.run(
                [
                    sys.executable,
                    str(DERIVE_PATH),
    ...
    def test_legacy_detector_recognizes_only_212_as_current(self) -> None:
        for version, expected_code, expected_status in (
            ("2.2.0", 0, "CURRENT_NO_MIGRATION_REQUIRED"),
