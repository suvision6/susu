#!/usr/bin/env python3
"""Formal director-shot-data/3.1.0 backend shipped by su-fenjingskill 3.1.1.
...
def validate_data(data: Any) -> dict[str, Any]:
    ...
    for index, scene in enumerate(scenes):
        ...
        errors.extend(
            production_risk_issues(
                scene.get("production_risks"),
                f"{path}.production_risks",
            )
        )
    ...
    for index, shot in enumerate(shots, start=1):
            ...
            if source_has_cjk and not contains_cjk(execution_text):
                errors.append(
                    issue(
                        ...
                        "中文来源的第五列必须包含可执行中文，不能只交付英文或内部代码。",
                    )
                )
            ...
            if leaked_terms:
                errors.append(
                    issue(
                        ...
                        f"{path}.execution_text",
                        "第五列泄露内部枚举：{}。".format(", ".join(leaked_terms)),
                    )
                )
        ...
        errors.extend(
            production_risk_issues(
                shot.get("production_risks"),
                f"{path}.production_risks",
            )
        )
        notes = normalize_text(shot.get("notes"))
        note_policy = remarks_policy(notes)
        if note_policy == "production-risk":
            errors.append(
                issue(
                    "PRODUCTION_RISK_IN_REMARKS",
                    f"{path}.notes",
                    "制作风险必须移入 production_risks[]，不得写入六列分镜的备注。",
                )
            )
        elif note_policy == "review":
            warnings.append(
                issue(
...
def render_markdown(data: dict[str, Any], report: dict[str, Any]) -> str:
    ...
    for scene in scenes:
        ...
        for risk in scene.get("production_risks", []):
            if normalize_text(risk):
                risk_entries.append(
                    (str(scene.get("scene_id", "SC???")), normalize_text(risk))
                )
    for shot in shots:
        if not isinstance(shot, dict):
            continue
        for risk in shot.get("production_risks", []):
            if normalize_text(risk):
                risk_entries.append(
                    (str(shot.get("shot_id", "SH???")), normalize_text(risk))
                )
    if risk_entries:
        lines.extend(["", "## 制作风险（不进入备注列）", ""])
        for owner, risk in risk_entries:
            lines.append(f"- **{owner}**：{risk}")
