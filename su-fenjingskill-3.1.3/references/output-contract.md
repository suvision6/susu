# Output Contract｜director-shot-data/3.1.3
...
内部来源台账、事实、镜头绑定、Gate 和确认事件属于 `director-workspace/3.1.3`，不进入正式数据文件。历史合同只能通过显式迁移生成新目录中的 3.1.3 draft，不能只改版本字符串。
...
camera.shot_size_reason / angle_reason
