# Internal Workspace Contract｜director-workspace/3.1.3

`director-workspace/3.1.3` 是来源、导演工作和正式 readiness 的内部事实载体，不是第五个正式交付文件。机器结构唯一权威是 [director-workspace.schema.json](../schemas/director-workspace.schema.json)。
...
`approval_events[]` 保存对应内容哈希的显式状态转换和事件哈希链。`lock` 只重算并按唯一依赖矩阵作废旧审批；不会自动批准任何 Gate 或 Alignment。

## 正式边界

只有 workspace Schema PASS、来源对齐 PASS、Gate 2 confirmed、Alignment passed 且对应确认事件匹配当前内容哈希，才能进入正式 `build-all`。workspace 不进入正式 XLSX，不增加工作表，不成为正式第五文件；风险、安全与治理内容也不进入创作流程或四文件。
