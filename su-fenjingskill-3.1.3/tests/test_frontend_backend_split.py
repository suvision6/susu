...
class FrontendBackendContractTests(unittest.TestCase):
    ...
    def test_contract_identity_is_3_1_3(self) -> None:
        self.assertEqual("3.1.3", self.example["contract_version"])
        self.assertEqual("3.1.3", self.example["source_skill_version"])
        self.assertEqual("director-workspace/3.1.3", self.workspace["workspace_contract"])
        ...
        self.assertEqual("director-shot-data/3.1.3", formal_schema["title"])
        self.assertEqual("3.1.3", formal_schema["properties"]["contract_version"]["const"])
        self.assertEqual("3.1.3", formal_schema["properties"]["source_skill_version"]["const"])
        self.assertEqual("director-workspace/3.1.3", workspace_schema["title"])
            ...
            for index, unit in enumerate(strategy["topology"]):
                ...
                for shot_id in unit["shot_refs"]:
                    ...
                    shot["viewpoint"] = {
                        ...
                        for key, value in viewing.items()
                        if key != "framing_intent" and key not in {"visible_subjects", "offscreen_subjects"}
                    }
                    shot["camera"].update(
                        {
                            "framing_mode": "space",
