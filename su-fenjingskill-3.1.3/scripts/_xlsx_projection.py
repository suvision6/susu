#!/usr/bin/env python3
"""Human-facing XLSX projection for director-shot-data/3.1.3.
...
_BACKEND_LEAK_PATTERNS = (
    re.compile(r"【(?:摄影|调度与表演|声音|剪辑|连续性|镜头动机)】"),
...
def render_xlsx_execution_text(
    ...
    sentences: list[str] = []
    for flow_index, (owner, resolved) in enumerate(validate_shot_flow(shot)):
...
def project_xlsx(data: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        "title": title,
        "summary": f"共 {len(storyboard_rows)} 镜｜总时长 {_format_duration(total_duration)} 秒",
