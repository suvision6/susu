#!/usr/bin/env python3
"""Internal source-alignment, director-method, and Gate review for su-fenjingskill 3.1.3.

director-workspace/3.1.3 is an internal audit carrier. It never becomes a fifth
...
def validate_camera_grammar(
    ...
    for strategy_index, strategy in enumerate(scene_strategies):
        ...
        reason_owners: Dict[str, Dict[str, Set[str]]] = {
            "viewing": {}, "shot_size": {}, "angle": {}, "movement": {},
        }
...
def lock_workspace(workspace: Any, shot_data: Any = None) -> Dict[str, Any]:
    ...
    gate_1_changed = bool(changed_keys & {"source_hash", "source_model_hash", "method_hash"})
        ...
        if workspace.get("workspace_contract") != WORKSPACE_CONTRACT:
            schema_errors.append(
                make_issue(
                    ...
                    "workspace_contract",
                    "内部工作区必须使用 director-workspace/3.1.3。",
                )
            )
...
def confirm_gate_2(
    ...
    *,
    ...
    if (
        updated.get("review_lock", {}).get("gate_1_status") != "passed"
        or gate_1_content_hash(hashes) not in approval_hashes["gate_1_approved"]
    ):
        raise ValueError("Gate 2 确认前必须先通过当前内容的 Gate 1")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="验证并操作 su-fenjingskill 3.1.3 的来源、导演方法、Gate 与 alignment 审批链。"
    )
    ...
    validate_parser = subparsers.add_parser("validate", help="验证内部工作区与正式 3.1.3 shot data。")
