...
def execution_basis(packet: dict[str, Any]) -> dict[str, Any]:
    """Bind authored execution inputs, not a claim about provider delivery."""
    scope = packet['execution_scope']
    return {
        'strategy': scope['strategy'], 'scope_status': scope['status'],
        'prompt_sha256': digest(packet['prompt'].encode('utf-8')),
        'references': [{k: r[k] for k in ('label', 'purpose', 'sha256')}
                       for r in packet['reference_files']],
        'provider_payload_verification': 'not_performed',
    }
...
def bundle_payloads(master: Path, output_dir: Path, director: Path | None = None,
    ...
    for page, mapped in zip(data['pages'], review['page_map']):
        ...
        basis = execution_basis(packet)
        packet['execution_basis'] = basis
        page['execution_basis'] = basis  # Derived plan only; the master stays unchanged.
...
def verify_saved_set(folder: Path) -> dict[str, Any]:
    ...
    plan = json.loads((folder / 'image-plan.json').read_text('utf-8'))
    plan_pages = {p['id']: p for p in plan['pages']}
    ...
    for item in manifest['pages']:
        ...
        planned = plan_pages.get(item['page'], {})
        if any('execution_basis' in obj for obj in (item, packet, planned)):
            # Old sets lacking all three optional bindings retain their old checks.
            expected_basis = execution_basis(packet)
            if any(obj.get('execution_basis') != expected_basis for obj in (item, packet, planned)):
                raise FormatError('页面执行输入与审阅依据不一致：' + item['page'])
