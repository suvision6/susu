...
SCOPE_STRATEGY = 'su-image9-execution-scope/1'
ANCHOR_SECTIONS = (
    '核心命名人物身份锚', '整场空间骨架',
    '人物位置与状态更新', '参考采用与统一媒介',
)
...
def scoped_context(context: str) -> tuple[str, dict[str, Any]]:
    """Remove only the explicitly delimited state archive from execution text.

    Do not infer time, solve spatial relationships, or rewrite any source prose.
    Legacy or ambiguous structures retain the old full-context behaviour.
    Document handles fenced Markdown so headings inside prose are not selectors.
    """
    prefix = '# execution-context\n'
    doc = Document(prefix + context)
    nodes = [h for h in doc.headings if h.level == 3]
    titles = [h.title for h in nodes]
    clear = (len(nodes) == len(ANCHOR_SECTIONS) and set(titles) == set(ANCHOR_SECTIONS)
             and not any(h.level == 2 for h in doc.headings)
             and all(doc.text[h.body:h.end].strip() for h in nodes))
    if not clear:
        return context, {'strategy': SCOPE_STRATEGY, 'status': 'legacy_full_context',
                         'state_selection': 'not_performed_by_script',
                         'note': '旧稿小节不完整或含未知同级栏目，保留原上下文；未启用作用域隔离。'}
    history = next(h for h in nodes if h.title == '人物位置与状态更新')
    start, end = history.start - len(prefix), history.end - len(prefix)
    filtered = context[:start] + context[end:]
    return filtered, {'strategy': SCOPE_STRATEGY, 'status': 'explicit_sections_scoped',
                      'excluded_section': history.title,
                      'state_selection': 'authored_in_page_and_panel_not_verified_by_script',
                      'note': '仅按明确标题隔离全场状态存档；当前状态是否完整仍须回读来源。'}
...
def prepare_request(master: Path, page_id: str, panel_id: str | None,
    ...
    execution_context, scope = scoped_context(data['context'])
    scoped = scope['status'] == 'explicit_sections_scoped'
    if scoped and panel_id is not None:
        neighbors = []
    ...
    if panel_id is None:
        ...
        task = (f'只绘制第{page_id}页的格{panel_id}，输出单个完整镜头画面，不画九宫格。'
                '只执行目标格明确展开的状态与机位。最终九格拼版另行处理。')
        output_kind = 'single_panel_for_later_3x3_composition'

    parts = [PREVIZ_STYLE, task, '【共用视觉依据，保留原稿文字】\n' + execution_context]
    if panel_id is None or not scoped:
        parts.append('【本页范围与关系说明】\n' + page['source_scope'] + '\n' + page['body'])
    else:
        parts.append('【当前单格范围】\n第' + page['id'] + '页格' + str(panel_id) +
                     '；人物位置、朝向与持有关系仅取下面目标格明确写出的当前状态。')
    if assets:
    ...
    return {
        ...
        'shared_context': data['context'],  # Archive, not an extra model input.
        'execution_scope': {**scope, 'included_neighbor_panels': len(neighbors),
                            'archive_only_fields': ['shared_context', 'page_context'],
                            'provider_payload_verified': False},
        ...
        'host_action_required': (
            ...
            '实际调用只使用prompt和真实附件，不把归档shared_context/page_context整段再次传入。'
            '当前格若仅写同上或依赖未展开状态，先在原视觉主稿中局部写清；不回写导演采用稿。'
            'legacy_full_context表示旧兼容行为，未证明状态隔离；提供新稿明确小节后再启用。'
...
def main() -> int:
    ...
    try:
        ...
        print(json.dumps({'saved': str(target), 'target': result['target'],
                          ...
                          'execution_scope': result['execution_scope']['status'],
