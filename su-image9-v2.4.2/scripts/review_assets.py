...
def build_catalog(data: dict[str, Any], namespace: str,
    ...
    for page in data['pages']:
        ...
        for panel in page['panels']:
            ...
            p['_execution_basis'] = copy.deepcopy(page.get('execution_basis'))
            p['slot'] = str(p.pop('id'))
    ...
    for p in flattened:
        ...
        spec.update(shared_context=data.get('context', ''), page_context=p['_page_context'], director_sha256=director_hash,
                    frame_ratio=ratio)
        if p['_execution_basis'] is not None:
            # Optional for legacy plans; new strategy or references invalidate old decisions.
            spec['execution_basis'] = p['_execution_basis']
        result.append({
