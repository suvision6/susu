...
SHOT_NOTE_LINE_PATTERN = re.compile(
    r"^(?P<character>[^｜\n]+)｜状态：(?P<state>[^；\n]+)；情绪：(?P<emotion>[^\n]+)$"
)
SHOT_NOTE_FORBIDDEN_PATTERN = re.compile(
    r"(?:\d+(?:\.\d+)?秒|时长|执行提醒|摄影机|机位|景别|构图|运镜|"
    r"轴线|安全|特效|曝光|焦点|镜头)"
)
...
def validate_shots(
    ...
    for index, shot in enumerate(shots):
        ...
        if note:
            if SHOT_NOTE_FORBIDDEN_PATTERN.search(note):
                result.error(
                    "SHOT_NOTE_CONTENT",
                    f"{path}.notes",
                    "备注不得复制第五列、写时长、摄影术语或内部执行提醒。",
                )
            if normalize_execution_text(note) in normalize_execution_text(
                shot.get("execution_text")
            ):
                result.error(
                    "SHOT_NOTE_DUPLICATES_PICTURE",
                    f"{path}.notes",
                    "第六列不得重复第五列画面内容。",
                )
