...
DIALOGUE_LANGUAGE_POLICY_MODE = "original_with_translation"
...
def validate_confirmations(data: dict[str, Any], result: ValidationResult) -> None:
    ...
    for key in sorted(actual - expected_keys):
        result.error("CONFIRMATION_UNKNOWN", f"$.confirmations.{key}", "不是 2.5.1 人工 Gate。")
...
def bilingual_dialogue_pairs(locked_text: str) -> list[tuple[int, int]]:
    ...
    for line_index, line in enumerate(lines):
        ...
        if family in {"han", "latin"}:
            ...
            candidates.append((line_index, family, is_italic_parallel))
    pairs: list[tuple[int, int]] = []
    for left, right in zip(candidates, candidates[1:]):
        left_index, left_family, left_parallel = left
        right_index, right_family, right_parallel = right
        ...
        if (
            ...
            and (left_parallel or right_parallel)
        ):
...
def validate_dialogue_language_policy(
    ...
    for key in sorted(DIALOGUE_LANGUAGE_POLICY_KEYS - actual_keys):
        result.error(
            "DIALOGUE_LANGUAGE_POLICY",
            f"$.source.dialogue_language_policy.{key}",
            "缺少双语台词语言策略字段。",
        )
    for key in sorted(actual_keys - DIALOGUE_LANGUAGE_POLICY_KEYS):
        result.error(
            "DIALOGUE_LANGUAGE_POLICY",
            f"$.source.dialogue_language_policy.{key}",
            "不是双语台词语言策略允许的字段。",
        )
    if policy.get("mode") != DIALOGUE_LANGUAGE_POLICY_MODE:
        result.error(
            "DIALOGUE_LANGUAGE_POLICY",
            "$.source.dialogue_language_policy.mode",
            f"必须为 {DIALOGUE_LANGUAGE_POLICY_MODE}。",
        )
...
def validate_beats(
    ...
    language_policy = (
        ...
        and raw_language_policy.get("mode") == DIALOGUE_LANGUAGE_POLICY_MODE
    ...
    *,
    ...
    if not isinstance(phases, list) or not phases:
        result.error(
            ...
            f"{path}.shot_phases",
            "2.5.1 每个镜头都必须用有序 shot_phases 说明真实观看进程。",
        )
        return int(duration)
...
def validate_duration_blocks(shot: dict[str, Any], path: str, result: ValidationResult) -> int | None:
    ...
    if not isinstance(blocks, list) or not blocks:
        result.error(
            ...
            "旧 duration_blocks 已由 2.5.1 shot_phases 替代。",
        )
        return duration
...
def validate_visual_plan_execution(
    ...
    *,
    ...
    for field_name in ("primary_subjects",):
...
def atomic_write_delivery(
    ...
    if any((output_dir / name).exists() for name in legacy_names):
        raise ValueError("输出目录含旧版固定文件名；2.5.1 必须使用带剧本前缀的新目录。")
