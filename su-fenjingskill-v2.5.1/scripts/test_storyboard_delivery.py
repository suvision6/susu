#!/usr/bin/env python3
"""Regression tests for the shot-data/2.5.1 delivery contract."""
...
def upgrade_draft_v240(draft: dict) -> dict:
    """Normalize legacy fixture prose into the director-and-DOP 2.5.1 contract."""
    draft["contract_version"] = "2.5.1"
    draft["source_skill_version"] = "2.5.1"
    ...
    for shot in draft.get("shots", []):
        ...
        spoken_text = "；".join(
            f'{item["speaker"]}以{item.get("delivery", item.get("shot_delivery", "onscreen"))}方式说：“{item["text"]}”'
            for item in dialogue
        )
        ...
        if shot["shot_id"] == "SH001":
            shot["notes"] = (
                "林｜状态：站在门口向周发问；情绪：克制试探。\n"
                "周｜状态：坐在桌边听问；情绪：警觉克制。"
            )
        elif shot["shot_id"] == "SH002":
            shot["notes"] = "周｜状态：抬眼并握紧钥匙；情绪：警觉转为承认。"
        elif shot.get("visible_characters"):
            character = shot["visible_characters"][0]
            shot["notes"] = f"{character}｜状态：延续本镜可见行动；情绪：保持当前情绪阶段。"
        else:
            shot["notes"] = ""
...
class StoryboardDeliveryTests(unittest.TestCase):
    ...
    def test_build_declares_contract_and_preserves_handoff_fields(self) -> None:
        ...
        self.assertEqual(built["contract_version"], "2.5.1")
        self.assertEqual(built["source_skill"], "su-fenjingskill")
        self.assertEqual(built["source_skill_version"], "2.5.1")
    ...
    def test_structured_character_notes_round_trip_as_text(self) -> None:
        draft = valid_draft()
        draft["shots"][0]["notes"] = (
            "林｜状态：站在门口完成问话；情绪：克制试探。\n"
            "周｜状态：坐在桌边承受问话；情绪：警觉克制。"
        )
        built, _, paths = self.build(draft)
        expected = delivery.expected_table_rows(built)
        self.assertEqual(delivery.read_markdown_rows(paths["markdown"]), expected)
        self.assertEqual(delivery.read_xlsx_rows(paths["excel"]), expected)
        self.assertEqual(expected[1][-1], draft["shots"][0]["notes"])

    def test_timing_and_execution_reminder_notes_are_rejected(self) -> None:
        draft = valid_draft()
        draft["shots"][0]["notes"] = (
        ...
        result = delivery.validate_data(prepared)
        self.assertIn("SHOT_NOTE_CONTENT", issue_codes(result))
    ...
    def test_entry_strategy_must_match_first_shot(self) -> None:
        draft = valid_draft()
        ...
        result = delivery.validate_data(self.prepared(draft))
        self.assertIn("SCENE_ENTRY_STRATEGY_MISMATCH", issue_codes(result))
    ...
    def test_rear_center_car_entry_requires_specific_directing_benefit(self) -> None:
        draft = valid_draft()
        entry = draft["scenes"][0]["directing_plan"]["entry_strategy"]
        entry.update(
            {
        ...
        camera["foreground_characters"] = []
        result = delivery.validate_data(self.prepared(draft))
        self.assertIn("MOVING_CAR_REAR_CENTER_DEFAULT", issue_codes(result))
