...
1. 锁定全文、批准修正、边界与 hash；缺少外部编号时按来源顺序生成稳定内部 ID。若检测到相邻双语台词候选，先锁定 `dialogue_language_policy`：来源有明确原文／译文标记时记录来源证据，否则暂停并取得用户确认，把确认同时写入 `approved_corrections`；主次未锁定不得进入 Gate 1。
