...
每镜字符串字段 `notes` 只记录画面中人物在该镜内的状态与情绪，不再生成或要求 `[时长估算]`、`[执行提醒]` 或任何同义前缀。推荐格式为：

```text
人物｜状态：可见或已建立的当前状态；情绪：可表演的当前情绪。
```

第六列只记录必要声音、摄影机可观察且影响观看的表演，以及影响后镜的连续性。纯环境镜、纯物件镜或没有必要附注时可为空。不得机械填写情绪、复制第五列、写时长或内部 ID。时长唯一由第四列、`duration_seconds` 与 `shot_phases[]` 承担。
...
```text
SH001 | 场景显示名 | B001～锁定原文 | 5秒 | 【近景｜平视｜固定】\n【画面内容】B 在桌边抬眼，画外问话落下后焦点仍停在他的面孔。 | B｜状态：承受画外问话；情绪：压抑而警觉。
```
...
- 备注来自必要声音、可观察且影响观看的表演，以及影响后镜的连续性；没有必要附注时可为空。
...
5. 重新执行原子事件、默认切镜、非切依据、长镜保护范围和摄影三元素审计，并校验身份、两 Gate、digest、场级风格锚点、逐镜视觉规划、来源覆盖、逐字对白、时长、连续性、规划／终稿一致、六列格式与禁字段。
...
机器不裁决镜头是否“更电影化”、表演审美或景别比例，但确定性审计发言权交接、事件原子性、普通镜头 10 秒上限、非切依据、遮挡保护范围和三元素纯度。`long_take.needs_review` 与有理由的连续性例外可保留为 WARN。
