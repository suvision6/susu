...
- 该规则只处理“原始台词＋对照译文”并列。若多种语言都是角色实际发言，必须按来源事实分别建 dialogue fact，不得伪装成译文关系。
...
- 存在 `dialogue_language_policy` 时，每个 dialogue fact 必须增加 `language` 与 `source_role=original_dialogue`；`language` 必须等于已锁定的 `original_language`，正文文字体系必须与该语言一致。译文不得成为 dialogue fact，也不得进入镜头 `dialogue[].text`。
