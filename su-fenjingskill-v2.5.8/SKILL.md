...
description: 将完整剧本、未编号剧本段落，或边界明确的连续场景片段转换为生产可执行的六列导演分镜和 shot-data/2.5.8 结构化交付。用于锁源、反向核对来源完整性、建立对白跨镜播放轨、完成导演风格和 DOP 镜头设计，并落实可播时长、镜头节奏、备注、连续性与四文件交付；不用于视频 Prompt、模型配置或合镜分组。
---
...
- 当前 Skill、UI 与正式数据合同版本统一为 `2.5.8`，Gate 2 规则修订号为 `2.5.8-rhythm-integrity-r1`。本次修订增加对白跨镜播放轨、表演说明分离、结构化动作／反应计时、场级节奏设计、极短镜／长镜门禁和节奏优先 Gate 2；旧 `2.5.3`、`2.5.4` 只保留只读验证兼容，不再生成。
...
| 数据与交付 | [output-contract.md](references/output-contract.md) | shot-data/2.5.8 到四文件的交付映射、两 Gate digest、六列与报告 |

结构与语义按以下优先级裁决：

1. `scripts/contract_schema.py` 及其导出的 [shot-data.schema.json](references/shot-data.schema.json) 唯一拥有 `shot-data/2.5.8` 字段、必填／可选、基础类型和闭合对象结构。
...
4. 只有用户看到最终 `director_profile` 后明确说“确认”，才把实际展示的 Gate 1 材料绑定到 digest；绑定成功后自动进入并展示 Gate 2，不等待“继续”。
   交互层必须把“最终 profile 已展示”显式传给确认意图辅助函数；展示前的 `STYLE-*`、序号或“就这个”只用于候选选择。

### 2. 整场规划与 Gate 2

1. Gate 1 后建立 Beat、受保护 facts 与逐字锁定的 `script_voice_type`；再以锁定原文清单反向核对所有叙事行、具名对白和来源声音。VO、OS 与现场对白不得根据相邻人物或反应对象推断；用户允许保留的未决声音使用 `unresolved`，并在备注写明候选值。
2. 把来源事实转译为原子 `screen_events[]`：先按发言权、主要观看主体、观看尺度、信息落点和动作发起者拆开，再讨论镜头。同一完整发言保持一个 dialogue fact；声音通过 `dialogue_playbacks[]` 连续跨镜，画面切换不得复制、漏写或改写台词。
...
5. 每场先提交 `rhythm_design`，再为每个规划单元完成 DOP 与结构化 `duration_design`：对白播放片段、动作步骤、反应停留、顺序／并行关系、最低可播时长、导演目标时长、节奏角色及理由。中文默认 3.5 字/秒并计算标点停顿；表演说明不计入口播。来源目标偏差超过 10% 必须复核，正向超时超过 50% 阻断 READY。
6. 景别不由物理距离机械决定。空间两端人物可分别使用近景或特写；检查的是既定空间、视线、轴线、银幕方向和切换是否成立。
7. Gate 2 先展示节奏红线、场次时长冲突、全部 1 秒镜、全部 11–19 秒长镜、对白跨镜播放轨、机械映射与模板风险，再展示视觉与声音策略、屏幕事件、切／留／重构地图、DOP 镜头表和执行风险；全部场次展示完成后只确认一次，并自动生成最终交付，不等待“继续”。
   入口空间是否真正建立、暂缓信息是否提前泄露及连续镜组是否兑现观察位置由 Agent 在此审核并交给 Gate；机器只执行结构、冲突、digest 与计划—终稿绑定，不使用大全景、构图模式或关键词代理。
...
4. 第五列镜头头严格为 `【景别｜角度｜运镜】`：景别、角度、运镜只写各自纯摄影元素。机位、构图、主体、焦点与空间策略保留在 DOP 结构及自然语言画面内容中。除通用标准术语和原剧本输入中逐字出现的内容外，画面描述词默认使用中文；不得把内部英文枚举或字段名写入正文。正文不得出现“所在区域”“处于主要观看位置”等模板套话。第六列备注默认留空；用户明确要求、真实待确认事项或必要人工复核可写入，并与 JSON、Markdown、Excel 保持一致。
   第五列不得逐字段拼接 DOP：每镜物理机位只自然写一次，角度与机位合并为可架设的观看关系；运动镜用一句连续过程说明触发、速度、路径和停止条件；`start_frame`、事件动作、焦点与 `end_frame` 语义重复时必须消重。主位、次要层、观看权、保留主要行动方向、按事件顺序、末项动作与当前可见结果等内部分析词不得进入生成性正文。对白以人物动作与冒号自然嵌入，正文逐字保留且不额外套用可能造成嵌套的引号。
...
- 1 秒镜必须提交 `short_shot_design`，说明独立观看价值、入点、出点及一秒可读理由；缺失即 FAIL。
- 普通剧情镜为 1–10 秒；11–19 秒必须使用含至少两个真实进展阶段和视觉／实时收益的 `long_take`。`monologue_delivery` 或 `testimony_statement` 不能单独证明画面不切。任何达到 20 秒的镜头一律 FAIL，不存在导演例外。
...
- 第六列备注只能承载用户明确要求、真实待确认事项或必要人工复核，不得塞入模板化时长说明、内部 ID 或普通执行正文。未决声音必须写明“待确认”及 V.O./O.S. 候选。
- 每个来源具名对白和声音必须分别建立 dialogue/sound fact，并进入 `sound_fact_ids[]` 与第五列；制作标签可排除，其他非空叙事行不得漏出 Fact 覆盖。
- `estimated_duration_seconds` 不得低于从 playback、动作步骤与反应停留重新计算的 `duration_design.minimum_total_seconds`，也必须兑现场级 `rhythm_design`；不允许用来源文字长度估动作时长，也不允许把最低下限机械当作最终镜长。
- 项目／场景不少于 8 镜时，1 秒、≤2 秒、长镜、最低值锁定、全切与单事件密度按固定 `rhythm_policy` 复核；导演目的、观看要求、切点理由或长镜审计达到 75% 模板复用时直接 BLOCKED。
- `duration_rationale`、DOP `motivation`、cut/hold/reframe 理由和场级 `rhythm_design` 必须针对当前事件与场景；不得复制统一理由或统一 3–10 秒区间。
- 不得按固定周期轮换景别、角度或运镜来制造“均衡分布”。均匀统计不是导演依据；检测到跨场周期轮换时直接 BLOCKED。
