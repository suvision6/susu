...
- `event_role` 只允许 `spatial | dialogue_turn | dialogue_continuation | action | reaction | reveal | object_detail | information_landing | transition`。一个完整发言只有一个 `dialogue_turn`；同一播放轨在后续画面继续时使用 `dialogue_continuation`，不增加发言权交接。
...
一个事件最多包含一个说话者。同一人物的一次完整对白不得因标点、停顿或排版换行拆成多个 fact；同一 fact 可以由一个 `dialogue_turn` 与多个 `dialogue_continuation` 画面事件承接，实际文字和时间由 playback segments 无缝分配。
...
`dialogue_rhythm` 只用于符合 [dialogue-staging.md](dialogue-staging.md) 的短对白回合例外；使用时必须在 `dialogue_design.justification` 中写明具体收益。

每次发言权从 A 转移到 B，双人或多人对白都默认 `cut`。只有上述依据与 `dialogue_design`、观看主体和 DOP 方案互相一致时才可不切。导演风格只能改变边界的节奏、距离与观看方法，不能用“停留感”抹掉必要覆盖。用户明确表达否定语义（要切、切开、换镜等）时，必须恢复为切镜。
...
cut、hold 与 reframe 分别按各自适用集合审计理由复用；任一模式不少于四项且同一 `trigger + director_reason` 覆盖达到 75%，即为模板坍缩。不能把大量相同 cut 与不同 hold 混合后用全体比例稀释。
...
- dialogue continuation 的 picture cut 不等于发言权改变；切点必须说明语义落点、倾听者反应、关系变化、动作或空间揭示收益，不得只写“长对白切镜”。

同场事件全部被覆盖、所有相邻边界都有决定、事件归属与决定一致，才能进入 Gate 2 确认。

不少于 12 镜时，Beat、Fact、Event、Planned Unit 与 Shot 数量完全镜像，且单事件单元与 cut 比例同时达到 90%，触发 `SOURCE_UNIT_SHOT_MIRRORING_REVIEW_REQUIRED`。不少于 8 个边界且 cut 比例达到 90%、单事件单元达到 90%、无 hold/reframe 时触发机械节奏复核。相同观看要求或切点理由达到 75% 属模板坍缩并直接 BLOCKED。
...
1. 节奏红线、时长分布与场次目标冲突；
2. 全部 1 秒镜、11–19 秒长镜及对白跨镜播放轨；
3. 机械映射、模板复用、floor-lock 与 cut／hold／reframe 密度；
4. 场景视觉与声音策略及屏幕事件；
5. 切／留／镜内重构地图与 DOP 镜头表；
6. 轴线、视线、空间与执行风险。
