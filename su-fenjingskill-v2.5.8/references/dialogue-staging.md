...
- 用户允许暂存、但来源不能区分 V.O./O.S. 时，两层都使用 `unresolved`；当前画面标记 `not_visible`，镜头备注必须写明“待确认”及 V.O./O.S. 候选。
...
- 同一说话者的完整发言只建立一个 dialogue fact 与一个 `dialogue_playback`。画面可在说话者、倾听者、关系、动作、物件或空间间切换；后续画面使用 `dialogue_continuation` 和连续 playback segment，不制造新发言权。
...
- 对白逐字保留，不得改写或省略。
- 每镜只写当前 playback segment 实际听到的逐字片段；全部片段按顺序拼接必须恰好等于完整 fact，不得在每镜复制全文。
...
- 对白行内明确的括号表演说明必须拆为 action fact，并在对应镜头阶段执行；不得进入 spoken text 或口播计时。歧义括号先阻断确认。
...
- 同一句对白不得按标点、呼吸或排版换行拆成多个 fact；picture cut 只能依据真实观看变化，并通过连续 playback segment 保持声音不断、全文不重不漏。

## 6. Gate 2 与终稿兑现

Gate 2 展示所有发言权交接、跨镜 playback、每段文字坐标、计划秒数、声音落位和 picture cut 收益；只有同镜包含多个对白轮次时才必须展开 `dialogue_design`。

终稿通过摄影机、【画面内容】段落、逐字对白和 `shot_delivery + speaker_presentation` 兑现规划。`shot_delivery` 闭合值为：

```text
onscreen | os | vo | mediated | unresolved
```
...
- playback segments 出现缺字、重字、倒序、时间断口、跨场或声音身份变化。
- continuation 镜复制整段对白，或 dialogue fact 内仍混入明确表演说明。
