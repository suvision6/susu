...
- 每场必需的 `directing_plan.rhythm_design`，把场景拆成连续节奏段，逐段登记节奏角色、目标镜长区间、剪辑密度意图和导演理由。
...
- 结构化 `duration_design` 与导演目标时长；对白播放片段、动作步骤和反应停留必须可重算。
- 抽象叙事目的。
...
Gate 2 先展示节奏红线、场次时长冲突、全部 1 秒镜、全部 11–19 秒长镜、对白跨镜播放轨、机械映射与模板风险，再展示视觉与声音策略、屏幕事件、切／留／重构地图、DOP 镜头表及轴线与执行风险。DOP 表继续逐镜显示完整 `movement_plan` 及观看主体、构图、焦点、起止画面和动机。确认前运行只读 `review-gate-2`：`READY` 可展示确认，`REVIEW_REQUIRED` 必须把警告与拟保留理由一起交给用户，`BLOCKED` 必须先修复。全部场次结束后只确认一次。
...
这里的“执行”由 Agent 在 Gate 2 展示和终稿复核中按连续镜组判断，用户通过 Gate 2 确认；机器只校验 entry 对象结构、required／withheld 冲突、digest 与规划—终稿绑定。`spatial_establish` 不等于大全景或 `environment`，移动汽车入口也不得靠后排中央或关键词白名单硬判。
...
正文只读取唯一 `execution_text`。旧版 `execution_passages[]` 只能作为 legacy 迁移材料，不是 2.5.8 权威正文。正式新建正文不再拆分【镜头调度】【人物表演与声音】【镜头结束】三段，而是写成一个连续的【画面内容】段落，顺序为：

```text
初始画面 → 镜内动作／焦点／摄影机变化 → 结束画面
```

### 4.1 结构化 DOP 到第五列的语义压缩

第五列不是 DOP 字段清单。终稿 camera 继续逐项绑定 Gate 2，但 `execution_text` 必须把这些事实压缩成一个现场可执行过程：

- 每镜物理机位只写一次。若 `camera.position` 已含水平朝向，正文不得再向同一目标重复“朝向／朝／面向”；角度与位置合并为一句可架设的关系，例如“机位略高于桌面，俯看摊开的地图”。
- 主次主体与构图转成具体可见关系，例如前景、后景、左右占位、遮挡和人物距离；不得把“主位、次要层、观看权、保留主要行动方向”等内部分析词直接写入正文。
- 运动镜只用一个连续句组完成触发、速度、路径与停止条件，不重复完整机位、起始动作或结束动作。固定镜头只说明保持的可见过程与结束状态。
- `start_frame`、事件动作、`focus_plan` 与 `end_frame` 语义相同或互相包含时只写一次；结束画面改用具体人物、道具、环境或焦点状态收束，不写“末项动作”“当前可见结果”等占位结论。
- 对白逐字保留，以人物动作、停顿和冒号自然嵌入。来源台词内部已有引号时不再额外套一层生成性引号。
- 同一场完成后必须连续通读生成性正文，检查同镜重复和跨镜模板坍缩；不能用通过结构校验代替自然语言复核。

构建器不自动编写自然语言，只验证 Agent 已提交的 `execution_text`。只精化正文且不改变已确认 visual plan 时，不重新 Gate 2；改变机位、构图、主体、焦点、运动计划或起止画面时仍须重新 Gate 2。
...
`shot-data/2.5.8` 的每个规划单元必须提交 `duration_design`：
...
overlap_mode = sequential | parallel
...
- 默认中文为 3.5 字/秒，空格分词语言为 2.5 词/秒；逗号／分号加 0.2 秒，句末加 0.4 秒，省略号／破折号加 0.6 秒。明确表演说明不计入口播。
- `speech_min_seconds` 由 validator 根据当前单元 playback segments 的逐字切片重算；同一完整 fact 可以跨镜，但所有片段必须无缝覆盖全文一次。
- `action_segments[]` 逐项绑定事件、开始条件、结束条件、最低执行秒数及 overlap group；`reaction_holds[]` 绑定反应事件、人物、可见变化和停留秒数。文字长度不能作为动作计时依据。
- 对白、动作、反应及 `minimum_total_seconds` 均由 validator 派生，不接受自报结果绕过。
- `editorial_target_seconds` 是导演镜长承诺，必须等于规划估算时长，且不得低于下限；`duration_rationale` 必须解释相对最低值的节奏选择。
- `pacing_role` 只允许 `establish | development | reaction | insert | transition | impact | long_take`，并须落入场级 `rhythm_design` 的连续节奏段。
- 项目／场景不少于 8 镜且 90% 镜头恰好锁在最低下限时进入 floor-lock 复核；不得把校验下限机械变成剪辑答案。
- `duration_rationale` 必须说明当前来源事件为什么需要该镜长；相同理由覆盖 75% 单元属于模板坍缩，不得用改写同义词绕过。
- 景别、角度、运镜和镜长不得按固定周期轮换以制造统计均衡；跨场重复周期命中 `DOP_PERIODIC_ROTATION_DETECTED` 后直接 BLOCKED。
- 每场 `rhythm_design` 必须由该场推进与观看转折派生；五场复用同一节奏段、同一镜长区间和同一理由属于 `SCENE_RHYTHM_TEMPLATE_COLLAPSE`。
- 场次目标偏差超过 10% 进入复核；正向超时超过 50% 阻断 READY。规划短于目标时禁止机械填时，只检查是否仍满足最低可播时长并说明主动缩短理由。

## 7. Long Take

普通剧情镜为 1–10 秒。1 秒镜必须提交 `short_shot_design`，说明 `accent | transition | impact | instantaneous_insert` 意图、独立观看价值、入点、出点及一秒可读理由。

11–19 秒必须显式写 `shot_form=long_take`，并提交：
  ...
  "temporal_progression": [
    {
      ...
      "phase_order": 1,
      "screen_event_ids": ["SEV008"],
      "start_condition": "人物开始压住呼吸",
      "visible_development": "身体重心逐步前移",
      "end_condition": "视线最终落定",
      "duration_seconds": 6
    }
  ]
}
...
`supports[]` 保留 `continuous_action | performance_development | spatial_progression | blocking_proof | real_time_tension | monologue_delivery | testimony_statement`，但 `monologue_delivery` 或 `testimony_statement` 不能单独成立；必须同时至少具备一项视觉或实时收益。

- `temporal_progression[]` 至少两个可区分阶段，逐项说明开始条件、可见发展、结束条件和秒数；总秒数必须等于镜头时长，并与最终 `shot_phases[]` 绑定。
- 仅因对白口播时间长不得选择长镜；完整 dialogue fact 应优先通过 playback segments 跨多个画面镜头连续播放。
- 任何达到 20 秒的镜头返回 `SHOT_DURATION_HARD_LIMIT`，不允许导演例外或 Gate 2 豁免。
...
- 状态只允许 `supported | needs_review`；11–19 秒长镜在 Gate 2 中逐镜展示。
- `needs_review` 产生 WARN。
- 包含说话者变化、多个观看主体、尺度跳变、认知落点或多个顺序动作且没有可执行连续进展时，不允许靠长镜理由保留，必须拆镜。
