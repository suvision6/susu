...
- 只有用户在看到最终 profile 后明确表达确认意图，才写入 `selected_style_option_id`、正式 `director_profile` 和 Gate 1 digest。
- **确认等价表达**：确认、确定、OK、同意、就这个、用 STYLE-01、可以、没问题、是的。
- **不确认表达**：继续、先往下、看看、比较一下、再想想、待定、选第二个看看（仅选择候选，不等于确认）。
...
交互层调用 `is_gate_1_confirmation_intent` 时必须显式传入“最终 profile 是否已经展示”的上下文。展示前的 `STYLE-*`、选择序号或“就这个”只参与候选选择；展示后才可按本节既有等价表达确认。该上下文不写入 `shot-data`，也不增加会话状态或人工 Gate。
...
结构权威与执行边界：Schema 拥有候选、profile 与锚点的字段、基础类型和闭合枚举；validator 校验候选数量／顺序、七段 rationale、profile 精确匹配、锚点引用和 digest；Agent 在展示时判断“四维至少两项实质差异”、场景适配与风险是否真实，Gate 1 由用户确认最终 profile。机器不得用字符串差异、关键词或分数替代这些语义判断。
